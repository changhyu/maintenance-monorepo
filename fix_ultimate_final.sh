...
# 모든 문제 해결 최종 스크립트 (모든 의존성 및 명령어 오류 해결)

echo "========== 프로젝트 모든 문제 최종 해결 ==========\n"

# 스크립트에 실행 권한 부여
echo "1. 스크립트에 실행 권한 부여..."
chmod +x fix_database.py fix_all_issues.py fix_config.py fix_npm_deps.sh fix_port.py restart_project.sh fix_concurrently.sh fix_npm_deps_force.sh fix_prisma_version.sh fix_file_saver_types.sh fix_google_maps_types.sh fix_script_command.sh
...
# Prisma 버전 수정
echo "5. Prisma 의존성 버전 수정..."
./fix_prisma_version.sh

# @types/file-saver 버전 수정
echo "6. @types/file-saver 버전 수정..."
./fix_file_saver_types.sh

# @types/google.maps 버전 수정
echo "7. @types/google.maps 버전 수정..."
./fix_google_maps_types.sh

# npm 스크립트 명령어 오류 수정
echo "8. npm 스크립트 명령어 오류 수정..."
./fix_script_command.sh

# npm 의존성 충돌 해결 (강제 옵션 사용)
echo "9. npm 의존성 충돌 강제 해결..."
