...
class GitService:
    ...
    def __init__(self, repo_path: str, max_workers: int = MAX_WORKERS):
        ...
        if not GIT_AVAILABLE:
            self.logger.warning(
                "GitPython 라이브러리가 설치되지 않아 제한된 기능으로 동작합니다."
            )
        ...
        self._cache_performance = {
            "hit_time_savings": 0.0,  # 캐시 적중으로 절약한 시간 (초)
            "operation_count": 0,  # 총 작업 수
            "cached_operations": 0,  # 캐시된 작업 수
            "total_time": 0.0,  # 총 소요 시간 (초)
        }

        # 캐시 키 패턴별 통계
        self._cache_key_patterns = {}

        # 캐시 항목 메트릭스
        self._cache_item_metrics = {}

        # 캐시 경고 임계값
        self._cache_warning_thresholds = {
            "low_hit_rate": 0.3,  # 30% 미만의 히트율
            "high_memory_usage": 0.9,  # 90% 이상의 메모리 사용
            "key_pattern_imbalance": 0.7,  # 패턴이 전체의 70% 이상 사용
            "stale_items": 0.5,  # 50% 이상의 항목이 오래됨
        }
        ...
        self._compression_enabled = (
            zlib is not None
        )  # zlib이 가용한 경우에만 압축 활성화
        self._process_monitoring_enabled = (
            psutil is not None
        )  # psutil이 가용한 경우에만 프로세스 모니터링 활성화
        ...
        try:
            ...
            if os.path.exists(self.repo_path):
                ...
                raise GitRepositoryException(
                    f"Git 저장소 경로가 존재하지 않습니다: {self.repo_path}"
                )
    ...
    def _clear_cache(self, key: Optional[str] = None):
        ...
        with self._cache_lock:
            if key is None:
                ...
                if keys_to_clear:
                    self.logger.debug(
                        f"'{key}' 관련 캐시 {len(keys_to_clear)}개가 초기화되었습니다: {keys_to_clear[:3]}"
                    )
                else:
    ...
    def _get_cached_data(self, key: str, ttl_key: str = "default"):
        ...
        # 압축된 데이터인 경우 압축 해제
        if (
            hasattr(self, "_compression_enabled")
            and getattr(self, "_compression_enabled")
    ...
    def _set_cache(self, key: str, data: Any, expiry: Optional[int] = None):
        ...
        with self._cache_lock:
            ...
            if (
                ...
                and self._get_object_size(data) > 10240
            ):  # 10KB 이상인 경우
                try:
                    ...
                    if (
                        self._get_object_size(compressed_data)
                        > self._get_object_size(data) * 0.8
                    ):
            ...
            if (
                new_size > self._max_cache_memory * 0.5
            ):  # 전체 캐시 메모리의 50% 이상인 경우
                if self._debug_cache:
                    self.logger.debug(
                        f"캐시 저장 무시 (너무 큰 항목): {key}, 크기: {new_size/1024:.1f}KB"
                    )
                return
            ...
            if (
                ...
                or self._current_cache_memory + new_size > self._max_cache_memory
            ):
            ...
            self._cache_size_map[key] = (
                new_size  # 크기 정보 즉시 저장하여 재계산 최소화
            )
            ...
            if self._disk_cache_enabled:
                ...
                should_save = (
                    is_test_data or is_important_data or random.random() < 0.05
                )
                ...
                if should_save:
                    try:
                        # 개별 항목 저장 (증분 캐싱인 경우)
                        if (
                            hasattr(self, "_disk_cache_incremental")
                            and self._disk_cache_incremental
                        ):
                            # 압축된 데이터는 압축 해제 후 저장 (디스크 전용 직렬화 이슈 방지)
                            if (
                                self._compression_enabled
                                and isinstance(compressed_data, bytes)
                                and compressed_data != data
                            ):
    ...
    def _get_cache(self, key: str) -> Optional[Any]:
        ...
        with self._cache_lock:
            if key in self._cache and key in self._cache_times:
                ...
                # 적응형 TTL 최적화
                if (
                    len(self._cache_access_patterns[key]) >= 10
                    and self._adaptive_cache_enabled
                ):
                    optimized_ttl = self._adaptive_cache_ttl(
                        key, self._cache_access_patterns[key]
                    )
                    key_prefix = key.split(":")[0] if ":" in key else key
    ...
    def get_status(self, use_cache: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ahead_behind = (
                    self._get_ahead_behind(current_branch)
    ...
    def _get_ahead_behind(self, branch: str) -> Dict[str, int]:
        ...
        try:
            ...
            remote_output = self._run_git_command(
                remote_branches_cmd, check_errors=False
            )
            remote_branches = (
                remote_output.splitlines() if remote_output.strip() else []
            )

            # 원격 브랜치가 없는 경우
            if not remote_branches:
                self.logger.debug(
                    f"브랜치 '{branch}'에 대응하는 원격 브랜치가 없습니다."
                )
            ...
            try:
                tracking_branch = self._run_git_command(
                    tracking_branch_cmd, check_errors=False
                ).strip()
            except Exception:
                # 추적 브랜치 없음
                return {"ahead": 0, "behind": 0}
    ...
    def _get_last_commit(self) -> Dict[str, str]:
        ...
        try:
            output = self._run_git_command(
                ["log", "-1", "--format=%H%n%an%n%ae%n%s%n%ci"]
            )
    ...
    def get_commit_history(
        ...
        try:
            ...
            for line in output.splitlines():
                ...
                parts = line.split("|")
                if len(parts) >= 5:
                    commit_hash, author_name, author_email, commit_time, subject = (
                        parts[:5]
                    )
            ...
            if use_cache:
                self._set_cache(
                    cache_key, commits, expiry=self._cache_ttl.get("commit_history")
                )
    ...
    def _get_detailed_commits_parallel(
        self, commits: List[Dict[str, Any]], max_workers: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        def process_commit_chunk(chunk):
            ...
            for commit in chunk:
                try:
                    ...
                    # 오류 발생 시 원본 커밋 정보 사용
                    self.logger.debug(
                        f"커밋 상세 정보 수집 오류: {commit['id']}, {str(e)}"
                    )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(process_commit_chunk, chunk) for chunk in commit_chunks
            ]
    ...
    def get_commit_stats(
        self, commit_hash: str = "HEAD", use_cache: bool = True
    ) -> Dict[str, Any]:
        """
        커밋 통계 조회
    ...
    def compare_branches(
        self, base_branch_name, compare_branch_name, use_cache: bool = True
    ):
        """
        두 브랜치 간의 차이를 비교합니다.
        ...
        try:
            ...
            if base_branch_name not in branches:
                raise GitOperationException(
                    f"기준 브랜치가 존재하지 않습니다: {base_branch_name}"
                )

            if compare_branch_name not in branches:
                raise GitOperationException(
                    f"비교 브랜치가 존재하지 않습니다: {compare_branch_name}"
                )
        ...
        try:
            ...
            # 공통 조상 커밋 찾기
            common_ancestor = self._run_git_command(
                ["merge-base", base_branch_name, compare_branch_name]
            ).strip()
            ...
            if different_files_output:
                for line in different_files_output.split("\n"):
                    ...
                    if len(parts) >= 2:
                        ...
                        file_info = {"path": file_path, "status": status}
    ...
    def _get_detailed_file_changes(
        self, files: List[Dict], base_branch: str, compare_branch: str, max_workers: int
    ) -> List[Dict]:
        """
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(process_file_chunk, chunk) for chunk in file_chunks
            ]
    ...
    def get_commit_contributors(self, use_cache: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            for line in output.strip().split("\n"):
                ...
                contributors.append(
                    {"name": name, "email": email, "commits": commits_count}
                )

            # 큰 저장소인 경우 병렬 처리로 기여자 상세 정보 수집
            if len(contributors) > 10 and optimal_workers > 1:
                contributors = self._get_detailed_contributors(
                    contributors, optimal_workers
                )
    ...
    def _get_detailed_contributors(
        self, contributors: List[Dict], max_workers: int
    ) -> List[Dict]:
        """
        ...
        # 기여자 목록을 청크로 나누기
        contributor_chunks = [
            contributors[i : i + chunk_size]
            for i in range(0, len(contributors), chunk_size)
        ]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(process_contributor_chunk, chunk)
                for chunk in contributor_chunks
            ]
    ...
    def pull(
        ...
        def _do_pull():
            ...
            try:
                ...
                # 특정 오류에 대한 구체적인 처리
                if (
                    "pull이 충돌 상태로 종료되었습니다" in e.stderr
                    or "CONFLICT" in e.stderr
                ):
                    conflicts = self._get_conflicts()
                    return {
                        "success": False,
    ...
    def _update_stats(
        ...
        if key not in self._cache_detailed_stats:
            self._cache_detailed_stats[key] = {
                "hits": 0,
                "misses": 0,
                "sets": 0,
                "time_saved": 0.0,
        ...
        if key in self._cache and key in self._cache_sizes:
            ...
            hit_rate = 0
            total_accesses = (
                self._cache_detailed_stats[key]["hits"]
                + self._cache_detailed_stats[key]["misses"]
            )
            if total_accesses > 0:
            ...
            metrics["efficiency"] = (hit_rate * time_saved_total) / (
                size / 1024
            )  # KB당 효율

            # 크기 대비 가치 비율
            metrics["size_value_ratio"] = time_saved_total / (
                size / 1024
            )  # KB당 절약 시간

    def _adjust_max_workers(self, task_complexity: int = 5) -> int:
        """
        작업 복잡도에 따라 최대 워커 수를 조정합니다.

        Args:
            task_complexity (int): 작업 복잡도 (1-10)
        ...
        # 기본 TTL 가져오기
        key_prefix = key.split(":")[0] if ":" in key else key
        base_ttl = self._cache_ttl.get(
            key_prefix, self._cache_ttl.get("default", 600)
        )  # 기본 10분
        ...
        try:
            ...
            frequency_factor = (
                max(1.0, min(5.0, 1.0 + math.log(access_count) / 2))
                if access_count > 0
                else 1.0
            )
            ...
            if recency > recency_threshold:
                # 오래된수록 감소 (지수적으로)
                recency_factor = max(
                    0.3, math.exp(-(recency - recency_threshold) / (avg_interval * 3))
                )
        ...
        with self._cache_lock:
            ...
            for path in alternative_paths:
                if path in self._file_cache_dependencies:
                    ...
                    if self._debug_cache:
                        self.logger.debug(
                            f"파일 변경으로 {len(cache_keys)}개 캐시 항목 무효화: {path}"
                        )
                    ...
                    for key in cache_keys:
                        ...
                        if key in self._cache_times:
                            del self._cache_times[key]
                        if key in self._cache_lru_queue:
                            self._cache_lru_queue.pop(
                                key, None
                            )  # OrderedDict에서 안전하게 제거
                        ...
                        if (
                            hasattr(self, "_cache_hit_count")
                            and key in self._cache_hit_count
                        ):
            ...
            if invalidated:
                ...
                if len(self._file_cache_dependencies) > 0 and (
                    len(self._file_cache_dependencies) % 10 == 0
            ...
            for file_path in file_paths:
                ...
                for path in paths_to_register:
                    ...
                    if self._debug_cache:
                        ...
                        self.logger.debug(
                            f"파일 캐시 의존성 등록: {path} -> {cache_key} (총 {dep_count}개 의존성)"
                        )
    ...
    def _optimize_dependency_map(self):
        ...
        with self._cache_lock:
            ...
            empty_paths = [
                path for path, deps in self._file_cache_dependencies.items() if not deps
            ]
            ...
            if empty_paths and self._debug_cache:
                self.logger.debug(
                    f"의존성 맵 최적화: {len(empty_paths)}개 빈 항목 제거"
                )
    ...
    def _get_changed_files_since_last_check(self) -> List[str]:
        ...
        try:
            ...
            if (
                not hasattr(self, "_last_checked_commit")
                or not self._last_checked_commit
            ):
                # 모든 추적된 파일 목록 가져오기
                all_files = self._run_git_command(
                    ["ls-files"], check_errors=False
                ).splitlines()
            ...
            commits_count = self._run_git_command(
                ["rev-list", "--count", commits_range], check_errors=False
            ).strip()
            ...
            if self._debug_cache and unique_files:
                self.logger.debug(
                    f"마지막 확인 이후 변경된 파일: {len(unique_files)}개"
                )
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        # 캐시 메모리 사용률
        memory_usage_pct = (
            (self._cache_size / self._max_cache_size) * 100
        ...
        performance_stats = {
            ...
            "avg_time_saving_per_hit": (
                (self._cache_performance["hit_time_savings"] / hits) if hits > 0 else 0
            ),
            "performance_improvement": (
                (
                    self._cache_performance["hit_time_savings"]
                    / self._cache_performance["total_time"]
                )
                * 100
                if self._cache_performance["total_time"] > 0
                else 0
            ),
        }
        ...
        for pattern, stats in self._cache_key_patterns.items():
            ...
            pattern_stats[pattern] = {
                ...
                "memory_pct": (
                    (stats["memory"] / self._cache_size) * 100
                    if self._cache_size > 0
                    else 0
                ),
                "hit_rate": (
                    stats["hits"] / total_pattern_requests
                    if total_pattern_requests > 0
                    else 0
                ),
                "time_saved": stats["time_saved"],
                "efficiency": (
                    stats["time_saved"] / (stats["memory"] / 1024 / 1024)
                    if stats["memory"] > 0
                    else 0
                ),
            }
        ...
        for pattern, pattern_data in pattern_stats.items():
            if (
                pattern_data["memory_pct"]
                > self._cache_warning_thresholds["key_pattern_imbalance"] * 100
            ):
                warnings.append(
                    f"불균형 캐시 사용: '{pattern}' 패턴이 메모리의 {pattern_data['memory_pct']:.1f}% 차지"
                )

        # 오래된 항목 확인
        stale_items = 0
        now = time.time()
        for key, last_accessed in [
            (k, self._cache_item_metrics.get(k, {}).get("last_accessed", now))
        ...
        if (
            stale_ratio > self._cache_warning_thresholds["stale_items"]
            and len(self._cache) > 5
        ):
            warnings.append(
                f"오래된 캐시 항목: 전체의 {stale_ratio:.1%}가 1시간 이상 접근 없음"
            )
        ...
        return {
            ...
            "warnings": warnings,
            "cached_since": (
                min(self._cache_creation_times.values())
                if self._cache_creation_times
                else 0
            ),
        }
    ...
    def _load_cache_from_disk(self):
        ...
        try:
            ...
            with self._cache_lock:
                ...
                sorted_items = sorted(
                    [
                        (k, self._cache_last_access_time.get(k, 0))
                        for k in self._cache.keys()
                    ],
                    key=lambda x: x[1],
                )
                    ...
                    if item_file.endswith(".item"):
                        try:
                            ...
                            if key and key not in self._cache:
                                ...
                                self._cache_times[key] = time.time()
                                self._cache_access_counts[key] = item_data.get(
                                    "access_count", 1
                                )
                                self._cache_last_access_time[key] = item_data.get(
                            ...
                            self.logger.error(
                                f"개별 캐시 항목 로드 중 오류: {item_file}, {str(e)}"
                            )

                if loaded_count > 0 and self._debug_cache:
                    self.logger.debug(
                        f"개별 캐시 항목 {loaded_count}개를 추가로 로드했습니다."
                    )
            ...
            if self._debug_cache:
                self.logger.debug(
                    f"디스크에서 캐시를 로드했습니다: {len(self._cache)}개 항목, "
    ...
    def _save_cache_to_disk(self):
        ...
        try:
            ...
            if self._debug_cache:
                cache_file_size = (
                    os.path.getsize(cache_file) if os.path.exists(cache_file) else 0
                )
                self.logger.debug(
                    f"캐시를 디스크에 저장했습니다: {len(self._cache)}개 항목, "
                    f"크기: {cache_file_size/1024:.1f}KB"
                )
                if saved_count > 0:
                    self.logger.debug(
                        f"중요 캐시 항목 {saved_count}개를 개별 저장했습니다. "
    ...
    def _save_item_to_disk(self, key: str, data: Any):
        ...
        try:
            ...
            with open(temp_item_file, "wb") as f:
                compressed_data = zlib.compress(
                    pickle.dumps(item_data, protocol=pickle.HIGHEST_PROTOCOL)
                )
                f.write(compressed_data)
            ...
            if self._debug_cache:
                file_size = (
                    os.path.getsize(item_file) if os.path.exists(item_file) else 0
                )
                self.logger.debug(
                    f"캐시 항목을 디스크에 저장했습니다: {key}, 크기: {file_size/1024:.1f}KB"
                )
    ...
    def _initialize_disk_cache(self):
        ...
        try:
            ...
            if loaded:
                self.logger.info(
                    f"디스크에서 {len(self._cache)}개의 캐시 항목을 로드했습니다."
                )
            else:
                ...
                for filename in os.listdir(items_dir):
                    ...
                    if file_age > max_age:
                        ...
                        try:
                            ...
                            self.logger.warning(
                                f"디스크 캐시 파일 삭제 실패: {item_path}, {str(e)}"
                            )

                if count > 0 and self._debug_cache:
                    self.logger.debug(
                        f"오래된 디스크 캐시 파일 {count}개 삭제 ({total_size/1024:.1f}KB 정리)"
                    )
            ...
            if os.path.exists(cache_file):
                ...
                if file_age > max_age:
                    try:
                        ...
                        if self._debug_cache:
                            self.logger.debug(
                                f"오래된 메인 캐시 파일 삭제 ({os.path.getsize(cache_file)/1024:.1f}KB)"
                            )
                    except Exception as e:
                        self.logger.warning(
                            f"메인 캐시 파일 삭제 실패: {cache_file}, {str(e)}"
                        )

        except Exception as e:
            self.logger.error(f"디스크 캐시 정리 중 오류 발생: {str(e)}")

    def _adaptive_cache_ttl(self, key: str, access_times: List[float]) -> int:
        """
        접근 패턴에 따라 TTL을 동적으로 조정합니다.

        Args:
            key: 캐시 키
            access_times: 접근 시간 기록
    ...
    def _get_object_size(self, obj: Any) -> int:
        ...
        try:
            ...
            if isinstance(obj, str):
                return len(obj.encode("utf-8")) + sys.getsizeof("")

            # 리스트, 세트, 튜플의 경우 각 요소의 크기 합산
            if isinstance(obj, (list, set, tuple)):
                return sys.getsizeof(obj) + sum(
                    self._get_object_size(item) for item in obj
                )

            # 딕셔너리의 경우 키와 값의 크기 합산
            if isinstance(obj, dict):
                return sys.getsizeof(obj) + sum(
                    self._get_object_size(k) + self._get_object_size(v)
                    for k, v in obj.items()
                )

            # pickle을 사용하여 직렬화 크기 계산 (다른 방법 실패 시)
            return len(pickle.dumps(obj, protocol=pickle.HIGHEST_PROTOCOL))

        except Exception as e:
            # 오류 발생 시 기본값 반환
            self.logger.debug(f"객체 크기 계산 중 오류: {str(e)}")
            return sys.getsizeof(obj) if obj is not None else 0
