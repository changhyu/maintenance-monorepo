...
from src.database.models import User, Shop, ShopService, ShopReview, ShopImage, Technician, Todo
...
from src.models.schemas import VehicleStatus, VehicleType, MaintenanceStatus, UserRole, ShopStatus, ShopType, ServiceType, TodoStatus, TodoPriority
...
def _create_maintenance_date(index: int, records_count: int) -> datetime:
    """정비 날짜를 생성합니다."""
    return datetime.now(timezone.utc) - timedelta(days=30 * (index + 1) + (records_count % 15))

def _create_todo_due_date(todos_count: int, base_days: int, variation_days: int) -> datetime:
    """할 일 마감일을 생성합니다."""
    return datetime.now(timezone.utc) + timedelta(days=base_days + (todos_count % variation_days))
...
def _initialize_database():
    ...
    print("기존 데이터 삭제 중...")
    tables = [
        Todo, MaintenancePart, MaintenanceDocument, Maintenance,
        Vehicle, ShopService, ShopReview, ShopImage,
        Technician, Shop, User
    ]
    for table in tables:
