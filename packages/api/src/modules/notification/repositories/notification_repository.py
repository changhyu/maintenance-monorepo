...
from packages.api.srcmodels.notification import Notification, NotificationStatus
...
class NotificationRepository(BaseRepository):
    ...
    async def update(self, notification: Notification) -> Notification:
        ...
        await self.collection.update_one(
            {"_id": ObjectId(notification.id)}, {"$set": notification_dict}
        )
        return notification
    ...
    async def get_pending_notifications(self, limit: int = 100) -> List[Notification]:
        cursor = (
            self.collection.find({"status": NotificationStatus.PENDING})
            .sort("created_at", 1)
            .limit(limit)
        )
