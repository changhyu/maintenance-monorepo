...
try:
    ...
    logger.error(
        "sqlalchemy 패키지가 설치되지 않았습니다. 데이터베이스 기능이 작동하지 않습니다."
    )
    from packagescore.dummy_modules import DummyFunc as func
    from packagescore.dummy_modules import DummySession
...
try:
    ...
    logger.error(
        "geopy 패키지가 설치되지 않았습니다. 거리 계산 기능이 작동하지 않습니다."
    )
    from packagescore.dummy_modules import geodesic

try:
    from packages.apidatabase import get_db
    from packagesdatabase.models import (Shop, ShopImage, ShopReview,
                                         ShopService)
except ImportError:
    logger.error(
        "database 모듈을 찾을 수 없습니다. 데이터베이스 액세스가 작동하지 않습니다."
    )
    from packagescore.dummy_modules import get_db
    ...
    # 더미 모델 정의
    Shop = DummyModel
    ShopService = DummyModel
    ShopReview = DummyModel
    ShopImage = DummyModel

from packagesmodels.schemas import (ShopCreate, ShopReviewCreate, ShopStatus,
                                    ShopUpdate)
...
def _get_shop_with_distance(
    shop: Shop, latitude: float, longitude: float
) -> Dict[str, Any]:
...
class ShopService:
    ...
    def get_shops(
        ...
        try:
            ...
            if location and all(
                k in location for k in ["latitude", "longitude", "distance"]
            ):
    ...
    def _enrich_shop_data(self, db: Session, shop: Any) -> Dict[str, Any]:  # type: ignore
        ...
        review_stats = (
            ...
            .filter(self.shop_review_model.shop_id == shop.id)
            .one()
        )

        shop_dict["rating"] = (
            float(review_stats.avg_rating) if review_stats.avg_rating else None
        )
        ...
        if data.services:
            for service_type in data.services:
                service = self.shop_service_model(
                    shop_id=new_shop.id, service_type=service_type
                )
                db.add(service)
    ...
    def update_shop(self, shop_id: str, data: ShopUpdate) -> Dict[str, Any]:
        ...
        if "business_hours" in update_data:
            shop.business_hours = [
                hours.dict() for hours in update_data.pop("business_hours")
            ]
        ...
        if "services" in update_data:
            ...
            for service_type in services:
                service = self.shop_service_model(
                    shop_id=shop_id, service_type=service_type
                )
                db.add(service)
    ...
    def get_shop_reviews(
        self, shop_id: str, skip: int = 0, limit: int = 100
    ) -> Dict[str, Any]:
        """정비소 리뷰 목록을 조회합니다."""
        db = get_db()
        shop = db.query(self.shop_model).filter_by(id=shop_id).first()
        ...
        reviews = (
            db.query(self.shop_review_model)
            .filter_by(shop_id=shop_id)
            .order_by(self.shop_review_model.created_at.desc())
            .offset(skip)
            .limit(limit)
            .all()
        )
        ...
        result_reviews = [
            {
                **review.__dict__,
                "user": (
                    ...
                    if (
                        user := db.query(self.user_model)
                        .filter_by(id=review.user_id)
                        .first()
                    )
                    else None
                ),
    ...
    def create_shop_review(
        self, shop_id: str, data: ShopReviewCreate
    ) -> Dict[str, Any]:
        """
        정비소 리뷰를 작성합니다.
    ...
    def find_nearby_shops(
        self, latitude: float, longitude: float, distance: float, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        근처 정비소를 검색합니다.
        ...
        stats_query = (
            ...
            .filter(self.shop_review_model.shop_id.in_(shop_ids))
            .group_by(self.shop_review_model.shop_id)
            .all()
        )
        ...
        services_query = (
            db.query(
                self.shop_service_model.shop_id, self.shop_service_model.service_type
            )
            .filter(self.shop_service_model.shop_id.in_(shop_ids))
            .all()
        )
        ...
        for shop in shops:
            ...
            if shop_distance <= distance:
                ...
                # 리뷰 정보 추가
                shop_stats = review_stats.get(
                    shop.id, {"rating": None, "review_count": 0}
                )
    ...
    def delete_shop_image(self, shop_id: str, image_id: str) -> bool:
        ...
        db = get_db()
        image = (
            db.query(self.shop_image_model)
            .filter_by(id=image_id, shop_id=shop_id)
            .first()
        )
    ...
    def check_shop_availability(
        self, shop_id: str, service_date: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise ValueError(
                "날짜 형식이 잘못되었습니다. YYYY-MM-DD 형식이어야 합니다."
            ) from parse_error
        ...
        if is_business_day:
            available_slots = [
                {"time": slot, "is_available": True} for slot in time_slots
            ]
    ...
    def get_shop_statistics(self) -> Dict[str, Any]:
        ...
        # 정비소 유형별 통계
        shop_type_stats = (
            db.query(Shop.type, sql_func.count(Shop.id).label("count"))
            .group_by(Shop.type)
            .all()
        )

        type_distribution = {
            str(shop_type): count for shop_type, count in shop_type_stats
        }

        # 서비스 유형별 통계
        service_type_stats = (
            db.query(
                ShopService.service_type, sql_func.count(ShopService.id).label("count")
            )
            .group_by(ShopService.service_type)
            .all()
        )

        service_distribution = {
            str(service_type): count for service_type, count in service_type_stats
        }
    ...
    def shop_model(self) -> Type[Shop]:
        """Shop 모델 반환"""
        try:
            from packagesdatabase.models import Shop

            return Shop
        except ImportError:
            logger.warning("Shop 모델을 찾을 수 없습니다. 더미 모델을 사용합니다.")
            from packagescore.dummy_models import DummyShop
    ...
    def shop_service_model(self) -> Type[ShopService]:
        """ShopService 모델 반환"""
        try:
            from packagesdatabase.models import ShopService

            return ShopService
        except ImportError:
            logger.warning(
                "ShopService 모델을 찾을 수 없습니다. 더미 모델을 사용합니다."
            )
            from packagescore.dummy_models import DummyShopService
    ...
    def shop_review_model(self) -> Type[ShopReview]:
        """ShopReview 모델 반환"""
        try:
            from packagesdatabase.models import ShopReview

            return ShopReview
        except ImportError:
            logger.warning(
                "ShopReview 모델을 찾을 수 없습니다. 더미 모델을 사용합니다."
            )
            from packagescore.dummy_models import DummyShopReview
    ...
    def shop_image_model(self):
        """ShopImage 모델 반환"""
        try:
            from packagesdatabase.models import ShopImage

            return ShopImage
        except ImportError:
            logger.warning("ShopImage 모델을 찾을 수 없습니다. 더미 모델을 사용합니다.")
            from packagescore.dummy_models import DummyShopImage
    ...
    def technician_model(self):
        """Technician 모델 반환"""
        try:
            from packagesdatabase.models import Technician

            return Technician
        except ImportError:
            logger.warning(
                "Technician 모델을 찾을 수 없습니다. 더미 모델을 사용합니다."
            )
            from packagescore.dummy_models import DummyTechnician
    ...
    def user_model(self):
        ...
        try:
            ...
            from packagescore.dummy_modules import DummyUser
...
def get_shop_statistics(db: Session, shop_id: str) -> Dict[str, Any]:  # type: ignore
    ...
    service_stats = {
        ...
        "average_price": (
            sum(s.price for s in services) / len(services) if services else 0
        ),
