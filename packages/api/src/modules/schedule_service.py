...
from sqlalchemy import and_, or_, func, desc, asc, case, literal

from ..models.schedule import (
    ScheduleModel, ScheduleReminderModel, ScheduleNoteModel, 
    ScheduleStatus, SchedulePriority, RecurrencePattern, ReminderType, ReminderStatus
)
...
UTC_TIMEZONE = "+00:00"
UPDATABLE_FIELDS = [
    "title", "description", "scheduled_date", "end_date", 
    "estimated_duration", "estimated_cost", "maintenance_type",
    "status", "is_recurring", "recurrence_pattern", "recurrence_interval",
    "priority", "assigned_to", "shop_id"
]
...
class ScheduleService:
    ...
    def _parse_datetime(self, date_str: str, field_name: str) -> datetime:
        ...
        try:
            ...
            raise ValidationException(f"{field_name} 형식이 올바르지 않습니다: {str(e)}") from e
    ...
    def _process_datetime_fields(self, data: Dict[str, Any]) -> None:
        ...
        if isinstance(data.get("scheduled_date"), str):
            data["scheduled_date"] = self._parse_datetime(data["scheduled_date"], "예약 일시")
        
        if data.get("end_date") and isinstance(data["end_date"], str):
            data["end_date"] = self._parse_datetime(data["end_date"], "종료 일시")

    def _process_reminders(self, schedule_id: str, reminders_data: List[Dict[str, Any]]) -> None:
        """알림 처리"""
    ...
    def _process_notes(self, schedule_id: str, notes_data: List[Dict[str, Any]]) -> None:
        """노트 처리"""
    ...
    def update_schedule(self, schedule_id: str, schedule_data: Dict[str, Any]) -> ScheduleModel:
        """일정 수정"""
        try:
    ...
    def add_schedule_note(self, schedule_id: str, content: str, created_by: Optional[str] = None) -> ScheduleNoteModel:
        """일정에 노트 추가"""
        try:
            # 일정 존재 여부 확인
    ...
    def _handle_completed_schedule(self, schedule: ScheduleModel) -> None:
        ...
        # 차량 정보 업데이트
        vehicle = self.db.query(VehicleModel).filter(VehicleModel.id == schedule.vehicle_id).first()
        if not vehicle:
            return
        ...
        if schedule.is_recurring and schedule.recurrence_pattern:
            try:
                ...
                if new_schedule := self._create_recurring_schedule(
                    schedule, next_date
                ):
                    logger.info(f"자동 생성된 일정: {new_schedule.id}, 차량: {vehicle.id}, 날짜: {next_date}")
            except Exception as e:
    ...
    def _create_recurring_schedule(self, original_schedule: ScheduleModel, next_date: datetime) -> Optional[ScheduleModel]:
        """반복 일정 생성"""
    ...
    def _calculate_next_schedule_date(
        ...
        if pattern_enum == RecurrencePattern.DAILY:
            ...
            if current_date.month == 2 and current_date.day == 29 and not calendar.isleap(target_year):
                return datetime(
                    target_year, 2, 28,
                    current_date.hour, current_date.minute, current_date.second
                )
    ...
    def get_schedule_reminders(self, schedule_id: str) -> List[ScheduleReminderModel]:
        """일정 알림 조회"""
        return self.db.query(ScheduleReminderModel).filter(
            ScheduleReminderModel.schedule_id == schedule_id
        ).order_by(asc(ScheduleReminderModel.reminder_time)).all()
    
    def get_schedule_notes(self, schedule_id: str) -> List[ScheduleNoteModel]:
        """일정 노트 조회"""
        return self.db.query(ScheduleNoteModel).filter(
            ScheduleNoteModel.schedule_id == schedule_id
        ).order_by(asc(ScheduleNoteModel.created_at)).all()
    ...
    def get_maintenance_statistics(self, vehicle_id: Optional[str] = None) -> Dict[str, Any]:
        """정비 통계 정보 조회"""
    ...
    def process_pending_reminders(self, limit: int = 50) -> int:
        """대기 중인 알림 처리"""
        now = datetime.now(timezone.utc)
        pending_reminders = self.db.query(ScheduleReminderModel).filter(
    ...
    def _send_reminder(self, reminder: ScheduleReminderModel) -> None:
        ...
        logger.info(f"알림 발송: {reminder.id}, 타입: {reminder.reminder_type}, 시간: {reminder.reminder_time}")
