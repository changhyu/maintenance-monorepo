...
class MaintenanceService:
    ...
    async def create_maintenance_record(self, data: Dict[str, Any]) -> MaintenanceRecord:
        """
        정비 기록 생성
    ...
    async def get_maintenance_record(self, record_id: str) -> MaintenanceRecord:
        ...
        Raises:
            ResourceNotFoundError: 정비 기록을 찾을 수 없는 경우
        """
        if self.db:
            record = await self.db.maintenance_records.get(record_id)
            if not record:
                raise ResourceNotFoundError(f"정비 기록을 찾을 수 없습니다: {record_id}")
            return record
        raise ResourceNotFoundError(f"정비 기록을 찾을 수 없습니다: {record_id}")
    
    async def update_maintenance_record(self, record_id: str, data: Dict[str, Any]) -> MaintenanceRecord:
        """
        정비 기록 업데이트
    ...
    async def schedule_maintenance(self, vehicle_id: str, date: datetime, description: str, 
                                   mileage: Optional[int] = None, cost: Optional[float] = None,
                                   performed_by: Optional[str] = None) -> MaintenanceRecord:
        """
        정비 일정 예약
    ...
    async def get_maintenance_by_id(self, maintenance_id: int) -> Optional[Dict[str, Any]]:
        """
        ID로 정비 작업 조회
