    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if "Authorization" in request.headers:
                ...
                if session_id:
                    session_result = (
                        await self.security_service.validate_session_activity(
                            user, session_id
                        )
                    )
                    if not session_result.success:
                        return Response(content=session_result.error, status_code=401)

                # 비밀번호 만료 확인
                password_result = await self.security_service.check_password_expiry(
                    user
                )
                if not password_result.success:
                    return Response(content=password_result.error, status_code=403)
                ...
                if await self.security_service.is_operation_sensitive(operation):
                    ...
                    security_result = (
                        await self.security_service.verify_operation_security(
                            user, operation, two_factor_code
                        )
                    )
                    if not security_result.success:
                        return Response(content=security_result.error, status_code=403)
            ...
            return Response(content="보안 검증 중 오류가 발생했습니다", status_code=500)
...
class CompressionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if (
            content_encoding
            or not any(ctype in content_type for ctype in self.compress_types)
        ...
        if hasattr(response, "body"):
            ...
            if len(body) >= self.minimum_size:
                ...
                logger.debug(
                    f"응답 압축: {len(body)} -> {len(compressed_body)} 바이트 ({compression_ratio:.1f}% 감소)"
                )
    ...
    async def _handle_batch_request(self, request: Request) -> JSONResponse:
        ...
        try:
            ...
            for idx, item in enumerate(batch_data):
                    ...
                    results.append(
                        {
                            "status": 400,
                            "body": {
                                "error": "각 배치 항목은 method와 path를 포함해야 합니다"
                            },
                            "id": item.get("id"),
                        }
                    )
                ...
                try:
                    start_time = time.time()
                    response = await self._process_subrequest(
            ...
            return JSONResponse(
                status_code=500, content={"error": f"배치 요청 처리 오류: {str(e)}"}
            )
