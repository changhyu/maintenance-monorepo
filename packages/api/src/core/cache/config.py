...
class CacheConfig(BaseModel):
    ...
    # 전역 설정
    enabled: bool = Field(default=True, description="캐싱 기능 활성화 여부")
    backend: CacheBackend = Field(
        default=CacheBackend.MEMORY,
        description="사용할 캐시 백엔드 (redis, memory, none)",
    )
    default_ttl: int = Field(default=300, description="기본 캐시 만료 시간(초)")

    # Redis 설정
    redis_url: Optional[str] = Field(
        default=None, description="Redis 연결 URL (예: redis://localhost:6379/0)"
    )
    ...
    redis_connection_pool_size: int = Field(
        default=10, description="Redis 연결 풀 크기"
    )
    redis_max_connections: int = Field(default=10, description="Redis 최대 연결 수")

    # 메모리 캐시 설정
    memory_max_size: int = Field(default=1000, description="메모리 캐시의 최대 항목 수")

    # API 캐싱 설정
    api_cache_enabled: bool = Field(
        default=True, description="API 응답 캐싱 활성화 여부"
    )
    api_default_ttl: int = Field(default=60, description="API 캐시 기본 만료 시간(초)")
    api_exclude_paths: List[str] = Field(
        default=[], description="캐싱에서 제외할 API 경로 패턴 목록"
    )

    # 함수 캐싱 설정
    function_cache_enabled: bool = Field(
        default=True, description="함수 결과 캐싱 활성화 여부"
    )
    function_default_ttl: int = Field(
        default=300, description="함수 캐시 기본 만료 시간(초)"
    )

    # 키 관련 설정
    key_prefix: str = Field(default="app", description="모든 캐시 키의 전역 접두사")

    # 기타 설정
    debug_mode: bool = Field(
        default=False, description="디버그 모드 활성화 여부 (로깅 증가)"
    )
