...
class EvictionPolicy(ABC):
    ...
    def get_eviction_candidates(
        self, cache: Dict[str, Dict[str, Any]], count: int = 1
    ) -> List[str]:
        """
        축출 대상 항목 키 목록 반환
...
class LRUEvictionPolicy(EvictionPolicy):
    ...
    def get_eviction_candidates(
        ...
        sorted_items = sorted(cache.items(), key=lambda x: x[1].get("last_accessed", 0))
...
class LRUCache:
    ...
    def __init__(
        self, max_size: int = 1000, eviction_policy: Optional[EvictionPolicy] = None
    ):
        """
        LRUCache 인스턴스 초기화
