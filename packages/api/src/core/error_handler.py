...
class ErrorContext:
    ...
    def __init__(self, exception: Exception, operation: str, error_code: Optional[str] = None):
    ...
    def _get_caller_info(self) -> Dict[str, Any]:
        ...
        caller_frames = [
            ...
            if frame.filename.endswith('.py') and 
               not frame.filename.endswith(('error_handler.py', 'exceptions.py'))
        ]
...
def get_error_message(error_code: str, lang: Optional[str] = None, **kwargs) -> str:
    ...
    if language not in ERROR_MESSAGES:
        language = "ko"
    template = ERROR_MESSAGES[language].get(error_code, ERROR_MESSAGES['ko']['UNKNOWN_ERROR'])
    try:
...
def _get_error_response_params(exception: Exception, operation: str, language: Optional[str], error_code_str: Optional[str]):
    if isinstance(exception, HTTPException):
        return (get_error_message("general_error", language, operation=operation),
                exception.detail,
                exception.status_code,
                error_code_str or f"HTTP_{exception.status_code}")
    elif isinstance(exception, ValidationError):
        return (get_error_message("validation_error", language, operation=operation),
        ...
        return (get_error_message("permission_error", language, operation=operation),
                str(exception),
                status.HTTP_403_FORBIDDEN,
                error_code_str or ErrorCode.PERMISSION_DENIED)
    elif isinstance(exception, KeyError):
        return (get_error_message("missing_data", language, operation=operation),
                str(exception),
                status.HTTP_400_BAD_REQUEST,
                error_code_str or ErrorCode.MISSING_KEY)
    elif isinstance(exception, TimeoutError):
        return (get_error_message("timeout_error", language, operation=operation),
                str(exception),
...
async def handle_exception(
    ...
    logger.error(f"{operation} 중 오류 발생", exc_info=True, extra=error_ctx.to_log_context())
...
async def error_response(
    ...
    # 요청 정보 추가 (개발 환경에서만)
    if request_info and (settings.DEBUG or settings.ENVIRONMENT.lower() == "development"):
