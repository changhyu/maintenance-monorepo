...
import logging
import time
from typing import Dict, List, Any, Callable, TypeVar, Optional, Set, Union

from .parallel_processor import ParallelProcessor, ParallelExecutionError, DataBatcher, AsyncQueryBatcher
from .resilience import retry, circuit_breaker, resilient, CircuitBreakerError
    ...
    async def execute_parallel_resilient(
        ...
        try:
            ...
            for key, error in e.errors.items():
                errors[key] = error
                logger.warning(f"작업 '{key}' 실행 실패 (continue_on_exception=True): {str(error)}")
            ...
            logger.info(f"탄력적 병렬 실행 완료 (일부 실패): {success_count}개 성공, {error_count}개 실패 "
...
class ResilientDataBatcher(DataBatcher):
    ...
    async def process_batches_resilient(
        ...
        logger.info(f"{len(items)} 항목을 {len(batches)}개 배치로 나누어 탄력적으로 처리합니다.")
        ...
        # 각 배치에 대한 작업 생성
        tasks = {f"batch_{i}": lambda batch=batch: processor(batch) 
            ...
            async with self.semaphore:
                try:
                    ...
                    if continue_on_exception:
                        logger.warning(f"쿼리 실행 실패 (continue_on_exception=True): {str(e)}")
                        return None
                    raise
...
async def parallel_map_resilient(
    ...
    # 결과를 원래 순서대로 정렬
    return [results.get(f"item_{i}") for i in range(len(items)) if f"item_{i}" in results] 