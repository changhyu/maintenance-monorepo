...
from typing import Set, Dict, Any, List, Callable, Coroutine, Awaitable, Optional
    ...
    while True:
        try:
            if hasattr(app.state, "cache_manager") and app.state.cache_manager:
                ...
                memory_usage = await app.state.cache_manager.check_memory_usage(threshold_mb=500)
                logger.info(f"캐시 메모리 사용량: {memory_usage['local_memory_human']}")
                
                # 임계값 초과 시 메모리 정리
                if memory_usage.get('threshold_exceeded', False):
                    logger.warning(f"캐시 메모리 사용량이 임계값을 초과했습니다. 정리 작업 시작...")
    ...
    while True:
        try:
            ...
            for task_name in list(_parallel_task_stats.keys()):
                ...
                last_update = datetime.fromisoformat(task_data.get('last_update', '2000-01-01T00:00:00'))
            ...
            if active_tasks > 0:
                total_executions = sum(data.get('total_executions', 0) for data in _parallel_task_stats.values())
                avg_execution_time = sum(data.get('avg_execution_time', 0) * data.get('total_executions', 0) 
                                      for data in _parallel_task_stats.values()) / max(total_executions, 1)
                
                logger.info(f"병렬 작업 모니터링: {active_tasks}개 작업, "
...
def get_parallelizable_tasks() -> List[str]:
    ...
    Returns:
        병렬 처리 가능한 태스크 이름 목록
    """
    return [name for name, task in TASK_REGISTRY.items() 
...
async def run_tasks_in_parallel(tasks: List[str], max_workers: int = None) -> Dict[str, Any]:
    """
    ...
    # 병렬 실행
    task_dict = {task_name: functools.partial(execute_task, task_name) for task_name in tasks}
