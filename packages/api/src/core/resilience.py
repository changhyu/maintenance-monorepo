...
class CircuitBreakerError(Exception):
    ...
    def __init__(self, service_name: str, failure_count: int, message: Optional[str] = None):
        self.service_name = service_name
        self.failure_count = failure_count
        msg = message or f"{service_name} 서비스 회로 차단기가 열려 있습니다 (실패 횟수: {failure_count})"
        super().__init__(msg)
...
class CircuitBreakerConfig:
    ...
    def is_counted_exception(self, exception: Exception) -> bool:
        ...
        return not any(isinstance(exception, exc_type) for exc_type in self.exclude_exceptions)
...
class CircuitBreaker:
    ...
    @classmethod
    def get_or_create(cls, name: str, config: CircuitBreakerConfig = None) -> 'CircuitBreaker':
        ...
        async with self._lock:
            ...
            if self.state == CircuitState.OPEN:
                ...
                if time.time() - self.last_failure_time >= self.config.recovery_timeout:
                    logger.info(f"회로 차단기 '{self.name}'이(가) HALF_OPEN 상태로 전환됩니다.")
        ...
        try:
            ...
            async with self._lock:
                if self.state == CircuitState.HALF_OPEN:
                    ...
                    if self.success_count >= self.config.success_threshold:
                        logger.info(f"회로 차단기 '{self.name}'이(가) CLOSED 상태로 복구되었습니다.")
                        self.state = CircuitState.CLOSED
                        self.failure_count = 0
                        self.success_count = 0
            ...
            async with self._lock:
                ...
                if self.config.is_counted_exception(e):
                    ...
                    if self.state == CircuitState.HALF_OPEN:
                        logger.warning(f"회로 차단기 '{self.name}'이(가) 반열림 상태에서 실패하여 OPEN 상태로 돌아갑니다.")
                        self.state = CircuitState.OPEN
                    
                    # 닫힌 상태에서는 실패 임계값을 확인
                    elif self.state == CircuitState.CLOSED and self.failure_count >= self.config.failure_threshold:
                        logger.warning(f"회로 차단기 '{self.name}'이(가) 실패 임계값에 도달하여 OPEN 상태로 전환됩니다.")
                        self.state = CircuitState.OPEN
    ...
    async def execute(self, func: Callable[..., Any], *args, **kwargs) -> Any:
        ...
        for attempt in range(self.config.max_retries + 1):  # 원래 시도 + 재시도
            try:
                ...
                # 재시도 여부 확인
                if not self.config.should_retry(e) or attempt >= self.config.max_retries:
                    # 더 이상 재시도 안 함
                    break
                ...
                logger.warning(f"함수 실행 실패 (시도 {attempt+1}/{self.config.max_retries+1}), "
        ...
        # 모든 재시도가 실패한 경우
        if last_exception:
            logger.error(f"모든 재시도 실패 ({self.config.max_retries+1}번 시도): {str(last_exception)}")
...
def retry(**kwargs):
    ...
    def decorator(func):
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            return event_loop.run_until_complete(retry_handler.execute(func, *args, **kwargs))
