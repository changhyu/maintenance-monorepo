...
from packages.api.src.core.exceptions import BadRequestException


class RateLimitExceeded(BadRequestException):
    """속도 제한 초과 예외"""
    def __init__(self, detail: str = "요청 횟수 제한을 초과했습니다. 잠시 후 다시 시도해주세요."):
        super().__init__(
            detail=detail,
            headers=None,
...
class RateLimiter(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        logger.info(f"속도 제한 미들웨어 초기화: 일반 사용자 {rate_limit_per_minute}회/분, 관리자 {admin_rate_limit_per_minute}회/분")
        if not enable_rate_limit:
            logger.warning("속도 제한이 비활성화되어 있습니다. 프로덕션 환경에서는 활성화하는 것이 좋습니다.")
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        allowed, current_count, reset_time = await self._check_rate_limit(
            client_id, 
            self.admin_rate_limit if is_admin else self.rate_limit
        )
    ...
    async def _check_rate_limit(self, client_id: str, limit: int) -> Tuple[bool, int, int]:
        """
        속도 제한 확인
    ...
    async def _check_rate_limit_redis(self, rate_key: str, limit: int, current_time: int) -> Tuple[bool, int, int]:
        """Redis 기반 속도 제한 확인"""
    ...
    def _check_rate_limit_memory(self, rate_key: str, limit: int, current_time: int) -> Tuple[bool, int, int]:
        """메모리 기반 속도 제한 확인"""
        # 새 시간 창으로 리셋
        ...
        return self._rate_limits[rate_key] <= limit, self._rate_limits[rate_key], self._rate_reset_time
