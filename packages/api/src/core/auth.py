...
from fastapi import Depends, Security, status, Header, Request
...
from packages.api.src.core.exceptions import UnauthorizedException, ForbiddenException
...
class TokenPayload(BaseModel):
    ...
    role: str = "user"  # 사용자 역할
    permissions: List[str] = []  # 권한 목록
    user_id: Optional[str] = None  # 사용자 ID (sub와 다를 수 있음)
...
def refresh_tokens(refresh_token: str) -> TokenResponse:
    ...
    try:
        ...
        if payload.get("type") != "refresh":
            raise UnauthorizedException(detail="액세스 토큰은 갱신에 사용할 수 없습니다")
...
async def get_current_user(
    ...
    Raises:
        UnauthorizedException: 인증 정보가 없거나 유효하지 않은 경우
    """
    if not credentials:
        raise UnauthorizedException(detail="인증 정보가 없습니다")
...
def has_permission(required_permissions: List[str] = None):
    ...
    async def permission_checker(current_user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
        ...
        if missing_permissions:
            ...
            logger.warning(f"권한 부족: 사용자 {current_user.get('sub')}에게 {permission} 권한이 없습니다")
            raise ForbiddenException(detail=f"이 작업을 수행하기 위한 권한이 부족합니다: {permission}")
...
def has_role(required_roles: List[str] = None):
    ...
    async def role_checker(current_user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
        ...
        if required_roles and user_role not in [role.lower() for role in required_roles]:
            logger.warning(f"역할 부족: 사용자 {current_user.get('sub')}의 역할이 {required_roles}에 포함되지 않습니다")
...
async def is_admin_user(request: Request) -> bool:
    ...
    try:
        ...
        return payload.get("role") == "admin" or "admin" in payload.get("permissions", [])
    except Exception:
        return False 