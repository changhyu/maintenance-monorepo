...
from packagesmodels.schemas import ApiResponse, PaginatedResponse, Pagination
...
class ApiResponse(BaseModel, Generic[T]):
    ...
    def success(
        ...
        response = cls(
            success=True, data=data, message=message, metadata=response_metadata
        )
    ...
    def pagination(
        ...
        return cls.success(data=items, message=message, metadata=response_metadata)
...
async def api_response(
    ...
    if (
        ...
        and isinstance(metadata["error"], dict)
    ):
...
async def paginated_response(
    ...
    limit = max(
        1, min(limit, getattr(settings, "MAX_PAGE_SIZE", 1000))
    )  # 1~MAX_PAGE_SIZE 사이
    ...
    pagination = Pagination(
        total=total, offset=offset, limit=limit, has_more=page < total_pages
    )
    ...
    # 직접 ApiResponse 생성
    return await api_response(data=items, meta=meta)


async def success_response(
    message: str, data: Optional[T] = None, meta: Optional[Dict[str, Any]] = None
) -> ApiResponse[T]:
    """
    ...
    return await api_response(data=data, meta=meta_with_message, status_code=200)


async def created_response(
    data: T, resource_id: str, resource_type: str, location: Optional[str] = None
) -> ApiResponse[T]:
    """
    리소스 생성 응답을 생성합니다.
    ...
    Returns:
        생성 성공 응답
    """
    meta = {"resource": {"id": resource_id, "type": resource_type}}

    if location:
        meta["resource"]["location"] = location
        meta["headers"] = {"Location": location}

    return await api_response(data=data, meta=meta, status_code=201)
...
async def no_content_response() -> ApiResponse[None]:
    ...
    Returns:
        콘텐츠 없음 응답
    """
    return await api_response(data=None, status_code=204)
