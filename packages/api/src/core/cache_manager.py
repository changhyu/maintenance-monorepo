...
from packages.api.src.corecache import cache
...
class EvictionPolicy:
    ...
    def _select_items_for_eviction(
        self, cache: Dict[str, Any], count: int
    ) -> List[str]:
        """
        제거할 항목을 선택하는 메서드
        ...
        if self.backend == CacheBackend.REDIS:
            try:
                if HAS_AIOREDIS:
                    ...
                    logger.warning(
                        "aioredis 모듈이 설치되지 않아 비동기 Redis 캐시를 사용할 수 없습니다."
                    )
                    self.redis_client = None
                ...
                if HAS_REDIS:
                    ...
                    logger.warning(
                        "redis-py 모듈이 설치되지 않아 동기 Redis 캐시를 사용할 수 없습니다."
                    )
                    self.sync_redis_client = None

                if not self.redis_client and not self.sync_redis_client:
                    logger.error(
                        "Redis 라이브러리가 설치되지 않아 메모리 캐시로 전환합니다."
                    )
        ...
        try:
            if self.backend == CacheBackend.REDIS:
                ...
                if keys_to_delete:
                    ...
                    logger.debug(
                        f"패턴에 일치하는 {len(keys_to_delete)}개 키 삭제 완료: {pattern}"
                    )
                ...
                keys_to_delete = [
                    k for k in self._memory_cache if compiled_pattern.match(k)
                ]
    ...
    async def keys(self, pattern: str = "*") -> List[str]:
        ...
        try:
            if self.backend == CacheBackend.REDIS:
                ...
                compiled_pattern = re.compile(pattern.replace("*", ".*"))
                return [
                    k for k in self._memory_cache.keys() if compiled_pattern.match(k)
                ]
...
def invalidate_cache_for(entity_type: str, entity_id: Optional[str] = None) -> None:
    ...
    # 비동기 태스크로 무효화 작업 실행
    asyncio.create_task(
        cache_manager.invalidate_resources(
            entity_type, [entity_id] if entity_id else None
        )
    )
