...
class EnvVarDefinition:
    ...
    def validate(
        ...
        try:
            ...
            # 값 범위 검증
            if self._is_numeric_type() and not self._validate_numeric_range(
                converted_value
            ):
                if self.min_value is not None and converted_value < self.min_value:
                    return (
                        False,
                        f"{self.name}의 값은 {self.min_value} 이상이어야 합니다.",
                        None,
                    )
                if self.max_value is not None and converted_value > self.max_value:
                    return (
                        False,
                        f"{self.name}의 값은 {self.max_value} 이하여야 합니다.",
                        None,
                    )
            ...
            if (
                ...
                and not re.match(self.regex, converted_value)
            ):
                return (
                    False,
                    f"{self.name}의 값이 패턴 {self.regex}에 맞지 않습니다.",
                    None,
                )
            ...
            if self.var_type == EnvVarType.PATH and not os.path.exists(converted_value):
                return (
                    False,
                    f"{self.name}에 지정된 경로가 존재하지 않습니다: {converted_value}",
                    None,
                )
            ...
            if self.choices and converted_value not in self.choices:
                ...
                return (
                    False,
                    f"{self.name}의 값은 다음 중 하나여야 합니다: {choices_str}",
                    None,
                )
            ...
            return (
                False,
                f"{self.name}의 값을 {self.var_type.value} 타입으로 변환할 수 없습니다: {str(e)}",
                None,
            )
...
class EnvValidator:
    ...
    def print_summary(self) -> None:
        ...
        logger.info(
            f"환경 변수 검증 결과: 전체 {total}개 중 {valid}개 유효, {missing}개 누락, {invalid}개 유효하지 않음"
        )
...
def validate_env() -> bool:
    ...
    if os.environ.get("ENVIRONMENT", "").lower() in ["development", "dev", "local"]:
        ...
        if "SECRET_KEY" not in os.environ:
            ...
            logger.warning(
                "SECRET_KEY가 없어 임시로 생성했습니다. 프로덕션 환경에서는 반드시 설정하세요."
            )
    ...
    if not success and os.environ.get("ENVIRONMENT", "").lower() in [
        "development",
        "dev",
        "local",
    ]:
        if (
            not env_validator.missing_vars
        ):  # 누락 변수는 없고 유효하지 않은 변수만 있는 경우
            logger.warning(
                "개발 환경에서 일부 환경 변수 유효성 검증 실패를 무시합니다."
            )
            return True
