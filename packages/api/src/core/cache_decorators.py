...
def json_dumps(data: Any) -> bytes:
    ...
    try:
        if USE_ORJSON:
            ...
            return orjson.dumps(
                data,
                option=orjson.OPT_SERIALIZE_NUMPY | orjson.OPT_SERIALIZE_DATACLASS | orjson.OPT_SORT_KEYS
            )
        else:
...
def compress_data(data: Any) -> Tuple[Any, bool]:
    ...
    try:
        ...
        if len(serialized) > COMPRESSION_THRESHOLD:
            ...
            # 압축률 계산 및 로깅
            compression_ratio = (len(serialized) - len(compressed)) / len(serialized) * 100
            if config.debug:
                logger.debug(f"데이터 압축: {len(serialized)} -> {len(compressed)} 바이트 ({compression_ratio:.1f}% 감소)")
...
def create_granular_cache_key(
    ...
    if cache_level == CacheLevel.HIGH:
        ...
        if relevant_headers:
            headers_str = ",".join(f"{k}={v}" for k, v in sorted(relevant_headers.items()))
        ...
        async def wrapper(*args, **kwargs):
            ...
            if isinstance(response, StarletteResponse):
                try:
                    ...
                    headers_dict["X-Cache-Key"] = cache_key[:50]  # 보안을 위해 잘라서 표시
                    ...
                    if compress:
                        ...
                        if is_compressed and config.debug:
                            logger.debug(f"응답 데이터 압축됨: {cache_key}, ID: {request_id}")
                    ...
                    if config.debug:
                        logger.debug(f"응답 캐싱 완료: {cache_key}, 만료: {effective_expire}초, ID: {request_id}")
        ...
        if response.status_code in (200, 201, 203, 204):
            try:
                ...
                if headers_dict.get("content-type") == "application/json":
                    ...
                    content = response_body.decode() if hasattr(response_body, "decode") else response_body
                ...
                if config.debug:
                    logger.debug(f"미들웨어 응답 캐싱 완료: {cache_key}, 만료: {self.expire}초")
