...
from packages.api.src.corelogging import security_logger


def generate_random_string(
    length: int = 32, include_special_chars: bool = False
) -> str:
    """
...
def encrypt_sensitive_data(data: str, key: Optional[str] = None) -> str:
    ...
    try:
        ...
        security_logger.warning(
            "cryptography 라이브러리가 설치되지 않았습니다. 암호화가 수행되지 않습니다."
        )
...
def decrypt_sensitive_data(encrypted_data: str, key: Optional[str] = None) -> str:
    ...
    try:
        ...
        security_logger.warning(
            "cryptography 라이브러리가 설치되지 않았습니다. 복호화가 수행되지 않습니다."
        )
