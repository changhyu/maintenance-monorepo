...
class CacheOptimizer:
    ...
    def __init__(self, data_dir: str = "data/cache_usage"):
        ...
        self.last_optimization = datetime.now() - timedelta(
            days=1
        )  # 처음 실행 시 최적화 작업이 가능하도록 설정
    ...
    async def load_usage_data(self) -> None:
        ...
        try:
            ...
            if os.path.exists(usage_file):
                ...
                logger.info(
                    f"{len(self.usage_data)} 개의 캐시 항목 데이터를 로드했습니다."
                )
            else:
                logger.info(
                    "저장된 캐시 사용 데이터가 없습니다. 새로운 데이터 수집을 시작합니다."
                )
    ...
    async def _optimize_all_caches(self) -> None:
        ...
        try:
            ...
            for cache_key, usage in self.usage_data.items():
                if (
                    len(usage.access_history) > 5
                ):  # 충분한 사용 데이터가 있는 경우만 최적화
    ...
    def schedule_optimization_tasks(self) -> None:
        ...
        if self._usage_data_save_task is None:
            self._usage_data_save_task = asyncio.create_task(
                self._periodic_save_usage_data()
            )
    ...
    async def record_cache_access(
        ...
        async with self._lock:
            # 최대 키 수 제한 확인
            if (
                key not in self.usage_data
                and len(self.usage_data) >= self.max_kept_keys
            ):
                # 가장 오래된 접근 키 제거
                oldest_key = min(
                    self.usage_data.items(), key=lambda x: x[1].last_hit_at
                )[0]
        ...
        async with self._lock:
            ...
            for prefix, usages in prefix_stats.items():
                ...
                avg_performance_gain = (
                    statistics.mean(performance_gains) if performance_gains else 0
                )
                ...
                # 최적화된 캐시 레벨 결정
                hourly_patterns = [
                    u.hourly_pattern for u in usages if u.access_count > 20
                ]
                if hourly_patterns:
                    # 시간별 접근 패턴의 표준 편차 계산
                    pattern_variance = 0
...
def with_optimized_cache(
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            # 최적화된 TTL 및 레벨 가져오기
            ttl = await get_cache_optimizer().get_optimal_ttl(
                cache_key, fallback_ttl or config.api_cache_ttl
            )
            level = await get_cache_optimizer().get_optimal_cache_level(
