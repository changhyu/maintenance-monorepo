    ...
    def _register_routes(self) -> None:
        ...
        async def list_items(
            ...
            **filters,
            ...
            # 캐시 키 생성
            cache_key = (
                f"{self.cache_key_prefix}:list:{skip}:{limit}:{sort_by}:{sort_order}"
            )
            ...
            result = paginate_list(
                items=items["items"], skip=skip, limit=limit, total_count=items["total"]
            )
        ...
        # 아이템 생성 라우트
        @self.router.post(
            "/", response_model=self.response_model, status_code=status.HTTP_201_CREATED
        )
