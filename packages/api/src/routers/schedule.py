...
from fastapi import APIRouter, Depends, Query, Path, Body, HTTPException, Response, Request
...
from ..models.schedule import ScheduleStatus, SchedulePriority, MaintenanceType
from ..core.cache import get_cache, ttl_cache
...
async def get_schedules(
    ...
    vehicle_id: Optional[str] = Query(None, pattern=UUID_PATTERN, description="차량 ID 필터"),
    shop_id: Optional[str] = Query(None, pattern=UUID_PATTERN, description="정비소 ID 필터"),
    ...
    if status and status not in VALID_STATUSES:
        raise HTTPException(status_code=400, detail=f"유효하지 않은 상태값입니다. 가능한 값: {', '.join(VALID_STATUSES)}")
    
    if maintenance_type and maintenance_type not in VALID_MAINTENANCE_TYPES:
        raise HTTPException(status_code=400, detail=f"유효하지 않은 정비 유형입니다. 가능한 값: {', '.join(VALID_MAINTENANCE_TYPES)}")
    
    if sort_by not in SORTABLE_FIELDS:
        raise HTTPException(status_code=400, detail=f"정렬 가능한 필드가 아닙니다. 가능한 필드: {', '.join(SORTABLE_FIELDS)}")
    
    if sort_order.lower() not in ["asc", "desc"]:
        raise HTTPException(status_code=400, detail="정렬 순서는 'asc' 또는 'desc'만 가능합니다.")
...
async def create_schedule(
    ...
    if hasattr(schedule_data, "priority") and schedule_data.priority not in VALID_PRIORITIES:
        raise HTTPException(status_code=400, detail=f"유효하지 않은 우선순위입니다. 가능한 값: {', '.join(VALID_PRIORITIES)}")
    
    if hasattr(schedule_data, "maintenance_type") and schedule_data.maintenance_type not in VALID_MAINTENANCE_TYPES:
        raise HTTPException(status_code=400, detail=f"유효하지 않은 정비 유형입니다. 가능한 값: {', '.join(VALID_MAINTENANCE_TYPES)}")
...
@cache_response(
    expire=300,  # 5분 캐시
    prefix="schedules_stats",
    include_query_params=True
)
async def get_maintenance_statistics(
    response: Response,
    vehicle_id: Optional[str] = Query(None, pattern=UUID_PATTERN, description="통계를 조회할 차량 ID"),
    controller: ScheduleController = Depends(ScheduleController)
):
    """
    정비 통계 정보를 조회합니다.
...
async def update_schedule(
    ...
    """
    # 유효성 검증
    if hasattr(schedule_data, "status") and schedule_data.status and schedule_data.status not in VALID_STATUSES:
        raise HTTPException(
            status_code=400, 
            detail=f"유효하지 않은 상태값입니다. 가능한 값: {', '.join(VALID_STATUSES)}"
        )
    
    if hasattr(schedule_data, "priority") and schedule_data.priority and schedule_data.priority not in VALID_PRIORITIES:
        raise HTTPException(
            status_code=400, 
            detail=f"유효하지 않은 우선순위입니다. 가능한 값: {', '.join(VALID_PRIORITIES)}"
        )
    
    result = await controller.update_schedule(schedule_id=schedule_id, schedule_data=schedule_data)
    if not result:
        raise HTTPException(status_code=404, detail=f"일정을 찾을 수 없습니다: {schedule_id}")
...
async def add_schedule_note(
    schedule_id: str = Path(..., pattern=UUID_PATTERN, description="노트를 추가할 일정 ID"),
...
@cache_response(
    expire=60,  # 1분 캐시
    prefix="schedule_notes",
    include_path_params=True
)
async def get_schedule_notes(
    response: Response,
    schedule_id: str = Path(..., pattern=UUID_PATTERN, description="노트를 조회할 일정 ID"),
    controller: ScheduleController = Depends(ScheduleController)
):
    """
...
@cache_response(
    expire=60,  # 1분 캐시
    prefix="schedule_reminders",
    include_path_params=True
)
async def get_schedule_reminders(
    response: Response,
    schedule_id: str = Path(..., pattern=UUID_PATTERN, description="알림을 조회할 일정 ID"),
    controller: ScheduleController = Depends(ScheduleController)
):
    """
...
async def update_schedule_status(
    schedule_id: str = Path(..., pattern=UUID_PATTERN, description="상태를 변경할 일정 ID"),
