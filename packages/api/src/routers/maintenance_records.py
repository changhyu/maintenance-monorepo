...
def get_maintenance_records(
    ...
    try:
        return maintenance_service.get_maintenance_records(skip=skip, limit=limit, filters=filters)
...
def complete_maintenance_record(record_id: str, request: CompleteMaintenanceRequest):
    try:
        return maintenance_service.complete_maintenance_record(record_id, mileage=request.mileage, cost=request.cost, notes=request.notes)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
...
def cancel_maintenance_record(record_id: str, request: CancelMaintenanceRequest):
    try:
        return maintenance_service.cancel_maintenance_record(record_id, reason=request.reason)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
...
@router.delete("/records/{record_id}/documents/{document_id}", response_model=Dict[str, Any])
def delete_maintenance_document(record_id: str, document_id: str):
    try:
        return {"deleted": maintenance_service.delete_maintenance_document(record_id, document_id)}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
...
def export_maintenance_records(
    ...
    try:
        result = maintenance_service.get_maintenance_records(skip=0, limit=1000, filters=filters)
        ...
        # CSV 헤더 작성
        writer.writerow(["id", "vehicle_id", "description", "date", "status", "cost", "performed_by", "notes"])
