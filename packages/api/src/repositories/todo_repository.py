...
from packagesmodels.schemas import (TodoCreate, TodoPriority, TodoResponse,
...
class TodoRepository(BaseRepository[Todo, TodoCreate]):
    ...
    def find_overdue_todos(self, user_id: Optional[str] = None) -> List[Todo]:
        ...
        try:
            logger.debug(
                f"기한 지난 Todo 조회 시작: user_id={user_id or '모든 사용자'}"
            )
    ...
    def find_upcoming_todos(
        ...
        try:
            logger.debug(
                f"다가오는 Todo 조회 시작: days={days}, user_id={user_id or '모든 사용자'}"
            )
            ...
            # 쿼리 생성
            query = self._get_active_todos_query()
            query = query.filter(and_(Todo.due_date >= now, Todo.due_date <= end_date))
    ...
    def batch_update_status(self, todo_ids: List[str], status: str) -> int:
        ...
        try:
            logger.debug(
                f"Todo 일괄 상태 업데이트 시작: {len(todo_ids)}개 항목, 상태={status}"
            )

            # 일괄 업데이트 수행
            result = (
                self.db.query(Todo)
                .filter(Todo.id.in_(todo_ids))
                .update(
    ...
    def get_scheduled_todos(self, days: int = 30) -> List[Todo]:
        ...
        try:
            ...
            query = self._get_active_todos_query()
            query = query.filter(
                Todo.due_date.isnot(None), Todo.due_date >= now, Todo.due_date <= future
    ...
    def get_todo_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            created_last_week = (
                self.db.query(Todo).filter(Todo.created_at >= week_ago).count()
            )
    ...
    def get_popular_tags(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 결과 형식화 및 제한
            result = [
                {"name": tag, "count": count} for tag, count in sorted_tags[:limit]
            ]
        ...
        try:
            ...
            if todo.metadata and "is_scheduled" in todo.metadata:
                ...
                if "scheduled_time" in todo.metadata:
                    todo.metadata["cancelled_time"] = datetime.now(
                        timezone.utc
                    ).isoformat()
                todo.updated_at = datetime.now(timezone.utc)
    ...
    def get_scheduled_tasks(self, from_time: datetime, to_time: datetime) -> List[Todo]:
        ...
        try:
            ...
            todos = self.db.query(Todo).filter(Todo.metadata.isnot(None)).all()
    ...
    def set_recurrence(
        ...
        try:
            logger.debug(
                f"Todo {todo_id} 반복 일정 설정 시작: {recurrence_type}, interval={interval}"
            )
    ...
    def cancel_recurrence(self, todo_id: str) -> Todo:
        ...
        try:
            ...
            if todo.metadata and "recurrence" in todo.metadata:
                ...
                todo.metadata["recurrence"]["cancelled_at"] = datetime.now(
                    timezone.utc
                ).isoformat()
                todo.updated_at = datetime.now(timezone.utc)
    ...
    def generate_next_recurring_todo(self, todo_id: str) -> Optional[Todo]:
        ...
        try:
            ...
            if (
                not source_todo.metadata
                or "recurrence" not in source_todo.metadata
                or source_todo.metadata["recurrence"].get("cancelled", False)
            ):
                return None
            ...
            if source_todo.due_date:
                if rec_type == RecurrenceType.DAILY:
                    ...
                    new_due_date = source_todo.due_date.replace(
                        year=new_year, month=new_month
                    )
                elif rec_type == RecurrenceType.YEARLY:
                    new_due_date = source_todo.due_date.replace(
                        year=source_todo.due_date.year + interval
                    )
            ...
            new_todo.metadata = {
                ...
                "recurrence_sequence": source_todo.metadata.get(
    ...
    def get_category_statistics(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = []
            categories = (
                self.db.query(Todo.category, func.count(Todo.id))
            ...
            for category, count in categories:
                ...
                for status in TodoStatus:
                    status_count = (
                        self.db.query(func.count(Todo.id))
                        .filter(Todo.category == category, Todo.status == status)
                        .scalar()
                    )
