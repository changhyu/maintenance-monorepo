...
from packagesmodels.schedule import (ScheduleModel, ScheduleNoteModel,
                                     SchedulePriority, ScheduleReminderModel,
                                     ScheduleStatus)
...
class ScheduleRepository(BaseRepository[ScheduleModel, Dict[str, Any]]):
        ...
        try:
            if schedule := (
                ...
                raise ScheduleNotFoundError(
                    f"일정 ID {schedule_id}를 찾을 수 없습니다."
                )
        except SQLAlchemyError as e:
    ...
    def get_upcoming_schedules(
        self, days: int = DEFAULT_UPCOMING_DAYS
    ) -> List[ScheduleModel]:
        """
        다가오는 일정 조회
    ...
    def get_schedules_by_status(
        self, status: ScheduleStatus, skip: int = 0, limit: int = 100
    ) -> Tuple[List[ScheduleModel], int]:
        """
        상태별 일정 조회
        ...
        try:
            ...
            return self._get_paginated_query_results(
                query, skip, limit, [joinedload(ScheduleModel.vehicle)]
            )
        except SQLAlchemyError as e:
            logger.error(f"상태 {status}의 일정 조회 중 오류: {str(e)}")
            return [], 0

    def get_schedules_by_shop(
    ...
    def get_schedules_by_date_range(
        ...
        try:
            ...
            return self._get_paginated_query_results(query, skip, limit, load_options)
        except SQLAlchemyError as e:
            logger.error(
                f"날짜 범위({start_date} ~ {end_date}) 기준 일정 조회 중 오류: {str(e)}"
            )
            raise ScheduleRepositoryError(
                f"날짜 범위 기준 일정 조회 중 오류 발생: {str(e)}"
            ) from e

    def _save_to_db(
        self, model: Any, operation: str, schedule_id: Optional[str] = None
    ) -> Optional[Any]:
        """데이터베이스 저장 공통 로직"""
        try:
            self.db.add(model)
            ...
            self.db.rollback()
            error_msg = (
                f"일정 ID {schedule_id}에 {operation} 중 오류"
                if schedule_id
                else f"{operation} 중 오류"
            )
            logger.error(f"{error_msg}: {str(e)}")
            return None

    def _log_error(
        self, operation: str, error: Exception, schedule_id: Optional[str] = None
    ) -> None:
        """
        에러 로깅을 위한 공통 메서드
    ...
    def add_reminder(
        self, schedule_id: str, reminder_data: Dict[str, Any]
    ) -> Optional[ScheduleReminderModel]:
        """일정에 알림 추가"""
        try:
            ...
            reminder = ScheduleReminderModel(schedule_id=schedule_id, **reminder_data)
            return self._save_to_db(reminder, "알림 추가", schedule_id)
        except SQLAlchemyError as e:
            return self._extracted_from_update_status_14(
                "알림 추가", e, schedule_id, None
            )

    def add_note(
        self, schedule_id: str, note_data: Dict[str, Any]
    ) -> Optional[ScheduleNoteModel]:
        """일정에 노트 추가"""
        try:
            schedule = self.find_by_id(schedule_id)
            if not schedule:
            ...
            note = ScheduleNoteModel(schedule_id=schedule_id, **note_data)
            return self._save_to_db(note, "노트 추가", schedule_id)
        except SQLAlchemyError as e:
            return self._extracted_from_update_status_14(
                "노트 추가", e, schedule_id, None
            )
    ...
    def get_pending_reminders(
        self, limit: int = DEFAULT_REMINDER_LIMIT
    ) -> List[ScheduleReminderModel]:
        """
        발송 대기 중인 알림 조회
        ...
        try:
            ...
            return (
                ...
                .options(joinedload(ScheduleReminderModel.schedule))
                .filter(
                    and_(
        ...
        try:
            ...
            for priority in SchedulePriority:
                count = base_query.filter(
                    ScheduleModel.priority == priority.value
                ).count()
    ...
    def bulk_update_status(
        self, schedule_ids: List[str], status: ScheduleStatus
    ) -> Tuple[bool, int]:
        """
            ...
            result = (
                ...
                .filter(
                    and_(
                        ...
                        ScheduleReminderModel.status.in_(
    ...
    def create_schedule_with_reminders(
        self, schedule_data: Dict[str, Any], reminders_data: List[Dict[str, Any]]
            ...
            with self.transaction() as session:
                ...
                for reminder_data in reminders_data:
                    reminder = ScheduleReminderModel(
                        schedule_id=schedule.id, **reminder_data
                    )
