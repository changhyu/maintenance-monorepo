...
from sqlalchemy import and_, or_, desc, asc, func, Index, event
from sqlalchemy.orm import Session, joinedload
from sqlalchemy.sql import text

from ..core.dependencies import get_db
from ..models.maintenance import MaintenanceModel, MaintenanceArchiveModel
from ..models.schemas import MaintenanceStatus, MaintenanceCreate, MaintenanceUpdate
from ..core.logging import get_logger
from ..core.metrics import track_db_query_time
from ..core.base_repository import BaseRepository, DatabaseOperationError, EntityNotFoundError, ValidationError
...
logger = get_logger("repositories.maintenance")
...
def cache_decorator(ttl: int = None):
    ...
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            # 캐시 키 생성 (함수명, 인자 기반)
            cache_key = f"maintenance:{func.__name__}:{hash(str(args))}{hash(str(kwargs))}"
...
class MaintenanceRepository(BaseRepository[MaintenanceModel, MaintenanceCreate]):
    ...
    def __init__(self, db: Session):
        ...
        Args:
            db: 데이터베이스 세션
        """
        super().__init__(db, MaintenanceModel)
    ...
    def _create_indexes(self):
        ...
        try:
            ...
            Index(
                ...
                MaintenanceModel.vehicle_id,
                MaintenanceModel.date
            ).create(self.db.bind)
    ...
    def _register_event_listeners(self):
        """이벤트 리스너를 등록합니다."""
        # 상태 변경 이벤트
        @event.listens_for(MaintenanceModel, 'after_update')
            ...
            if hasattr(target, '_sa_instance_state'):
                ...
                if history.has_changes():
                    ...
                    if old_status and new_status:
                        self.event_emitter.emit('maintenance_status_changed', {
    ...
    async def get_maintenance_by_id(self, maintenance_id: str) -> Optional[Dict[str, Any]]:
        """ID로 정비 기록 조회"""
        try:
    ...
    @track_db_query_time
    async def get_maintenance_by_vehicle_id(self, vehicle_id: str) -> List[Dict[str, Any]]:
        """차량 ID로 정비 기록 목록 조회"""
        try:
            records = (
                self.db.query(MaintenanceModel)
    ...
    async def get_maintenance_by_vehicle_id_and_status(
        ...
        try:
            ...
            raise DatabaseOperationError(f"차량 ID {vehicle_id}와 상태 {status_list}의 정비 기록 조회 중 오류: {str(e)}")
            
    @track_db_query_time
    async def count_maintenance_records(self) -> int:
        """전체 정비 기록 수 조회"""
        try:
            return await self.db.query(MaintenanceModel).count()
        except Exception as e:
    ...
    async def create_maintenance(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 이벤트 발행
            self.event_emitter.emit('maintenance_created', {
    ...
    @track_db_query_time
    async def update_maintenance(self, maintenance_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """정비 기록 업데이트"""
        try:
            # 데이터 유효성 검사
    ...
    async def delete_maintenance(self, maintenance_id: str) -> bool:
        """정비 기록 삭제"""
        try:
            maintenance = await self.db.query(MaintenanceModel)\
    ...
    async def get_maintenance_by_status(self, status: str, limit: int = 100) -> List[Dict[str, Any]]:
        """상태별 정비 기록 조회"""
        try:
            records = await self.db.query(MaintenanceModel)\
            ...
            raise DatabaseOperationError(f"상태 {status}인 정비 기록 조회 중 오류: {str(e)}")
    ...
    async def get_recent_maintenance(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            records = await self.db.query(MaintenanceModel)\
                .filter(MaintenanceModel.date >= from_date)\
                .order_by(MaintenanceModel.date.desc())\
                .all()
            
            return [self._model_to_dict(record) for record in records]
        except SQLAlchemyError as e:
            raise DatabaseOperationError(f"최근 {days}일간 정비 기록 조회 중 오류: {str(e)}")
    ...
    def get_maintenance_list(
        ...
        sort_order: str = "desc"
    ) -> Tuple[List[MaintenanceModel], int]:
        """
    ...
    @track_db_query_time
    def find_overdue_maintenance(self, vehicle_id: Optional[str] = None) -> List[MaintenanceModel]:
        """
        기한이 지난 유지보수 기록 조회
        ...
        Returns:
            List[MaintenanceModel]: 기한이 지난 유지보수 기록 목록
        """
        try:
            logger.debug(f"기한 지난 유지보수 기록 조회 시작: vehicle_id={vehicle_id or '모든 차량'}")
            
            # 기본 쿼리 생성 - 기한이 지난 항목
            query = self._get_active_maintenance_query()
            query = query.filter(MaintenanceModel.due_date < datetime.now(timezone.utc))
            
            # 차량 필터 적용
            if vehicle_id:
                query = query.filter(MaintenanceModel.vehicle_id == vehicle_id)
    ...
    @track_db_query_time
    def find_upcoming_maintenance(self, days: int = 7, vehicle_id: Optional[str] = None) -> List[MaintenanceModel]:
        """
        다가오는 유지보수 기록 조회
        ...
        Returns:
            List[MaintenanceModel]: 다가오는 유지보수 기록 목록
        """
        try:
            logger.debug(f"다가오는 유지보수 기록 조회 시작: days={days}, vehicle_id={vehicle_id or '모든 차량'}")
            ...
            query = query.filter(
                and_(
                    MaintenanceModel.due_date >= now,
                    MaintenanceModel.due_date <= end_date
                )
            )
    ...
    def batch_update_status(self, maintenance_ids: List[str], status: str) -> int:
        ...
        """
        try:
            logger.debug(f"유지보수 기록 일괄 상태 업데이트 시작: {len(maintenance_ids)}개 항목, 상태={status}")

            # 일괄 업데이트 수행
            result = self.db.query(MaintenanceModel).filter(
                MaintenanceModel.id.in_(maintenance_ids)
            ).update({
                'status': status,
            ...
            if status == MaintenanceStatus.COMPLETED:
                self.db.query(MaintenanceModel).filter(
                    MaintenanceModel.id.in_(maintenance_ids),
                    MaintenanceModel.completed_at.is_(None)  # 이미 완료 시간이 설정되지 않은 항목만
                ).update({
                    'completed_at': datetime.now(timezone.utc)
                }, synchronize_session=False)

            self.db.commit()
            logger.debug(f"유지보수 기록 일괄 상태 업데이트 완료: {result}개 항목 업데이트됨")
            return result
        except Exception as e:
    ...
    def get_maintenance_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 전체 유지보수 기록 수
            total_count = self.db.query(func.count(MaintenanceModel.id)).scalar()
            
            # 상태별 유지보수 기록 수
            status_counts = {}
            for status in MaintenanceStatus:
                count = self.db.query(func.count(MaintenanceModel.id)).filter(
                    MaintenanceModel.status == status
                ).scalar()
                status_counts[status.value] = count
            
            # 차량별 유지보수 기록 수
            vehicle_counts = self.db.query(
                MaintenanceModel.vehicle_id,
                func.count(MaintenanceModel.id).label('count')
            ).group_by(MaintenanceModel.vehicle_id).all()
            
            # 월별 유지보수 기록 수 (최근 12개월)
            now = datetime.now(timezone.utc)
            start_date = now - timedelta(days=365)
            monthly_counts = self.db.query(
                func.date_trunc('month', MaintenanceModel.created_at).label('month'),
                func.count(MaintenanceModel.id).label('count')
            ).filter(
                MaintenanceModel.created_at >= start_date
            ).group_by('month').order_by('month').all()
            
            return {
                'total_count': total_count,
                'status_distribution': status_counts,
                'vehicle_distribution': {str(vid): count for vid, count in vehicle_counts},
                'monthly_trend': {str(month.date()): count for month, count in monthly_counts}
            }
        except Exception as e:
    ...
    @track_db_query_time
    def batch_create_maintenance(self, data_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def search_maintenance(
        ...
        """
        try:
            logger.debug(f"유지보수 기록 검색 시작: search_term={search_term}, fields={fields}")
            
            # 검색 가능한 문자열 필드 목록
            searchable_fields = [
                'title', 'description', 'technician_notes', 
                'parts_used', 'service_type', 'location'
            ]
    ...
    def search_by_tags(self, tags: List[str], match_all: bool = False) -> List[Dict[str, Any]]:
        ...
        """
        try:
            logger.debug(f"태그 기반 유지보수 기록 검색 시작: tags={tags}, match_all={match_all}")
    ...
    def get_maintenance_cost_by_vehicle(
        ...
        try:
            ...
            cost_by_vehicle = {
                str(vehicle_id): float(total_cost)
                for vehicle_id, total_cost in results
            }
        ...
        try:
            ...
            results = (
                self.db.query(
                    ...
                    func.count(case([
                        (self.model.status == MaintenanceStatus.COMPLETED, 1)
    ...
    @track_db_query_time
    def bulk_insert_maintenance(self, records: List[Dict[str, Any]], chunk_size: int = 1000) -> int:
        """
        ...
        try:
            ...
            for i in range(0, len(records), chunk_size):
                ...
                # 벌크 삽입 실행
                self.db.bulk_insert_mappings(MaintenanceModel, chunk)
        ...
        try:
            ...
            for i in range(0, len(records), batch_size):
                ...
                for record in batch:
                    ...
                    maintenance_obj = MaintenanceModel(
                        id=record_id,
    ...
    def bulk_update_status(
        ...
        """
        try:
            logger.debug(f"정비 기록 상태 일괄 업데이트 시작: {len(maintenance_ids)}개 레코드")
            ...
            for i in range(0, len(maintenance_ids), batch_size):
                ...
                result = (
                    self.db.query(MaintenanceModel)
                    .filter(MaintenanceModel.id.in_(batch_ids))
                    .update(
                        {
                            MaintenanceModel.status: status,
                            MaintenanceModel.updated_at: current_time
                        },
                        synchronize_session=False
                    )
                )
    ...
    def find_duplicates(
        ...
        try:
            ...
            # 기본 쿼리 구성
            query = self.db.query(MaintenanceModel)
            ...
            query = query.filter(MaintenanceModel.created_at >= time_limit)
            ...
            for i, record1 in enumerate(records):
                for record2 in records[i+1:]:
                    # 모든 지정된 필드가 일치하는지 확인
                    if all(getattr(record1, field) == getattr(record2, field) 
                          for field in fields):
                        # 시간 차이 계산
                        time_diff = abs(
                            (record1.created_at - record2.created_at).total_seconds()
                        ) / 3600  # 시간 단위로 변환
    ...
    def validate_data_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 전체 레코드 수 계산
            report['total_records'] = self.db.query(MaintenanceModel).count()
            
            # 필수 필드 검증
            required_fields = ['vehicle_id', 'service_type', 'status']
            records = self.db.query(MaintenanceModel).all()
            ...
            for record in records:
                ...
                if record.vehicle_id:
                    vehicle_exists = self.db.query(
                        self.db.query(MaintenanceModel)
                        .filter(MaintenanceModel.id == record.vehicle_id)
                        .exists()
                    ).scalar()
        ...
        try:
            ...
            while True:
                ...
                records = (
                    ...
                    .filter(
                        and_(
                            MaintenanceModel.date < cutoff_date,
                            MaintenanceModel.status == MaintenanceStatus.COMPLETED
                        )
                    )
                    .limit(batch_size)
                    .all()
                ...
                for record in records:
                    ...
                    self.db.execute(
                        MaintenanceArchiveModel.__table__.insert(),
                        archived_data
                    )
                ...
                # 원본 데이터 삭제
                self.db.query(MaintenanceModel).filter(
                    MaintenanceModel.id.in_([r.id for r in records])
                ).delete(synchronize_session=False)
            ...
            logger.debug(f"오래된 유지보수 기록 아카이브 완료: {total_archived}개 처리됨")
    ...
    def cleanup_incomplete_records(self, hours: int = 24) -> int:
        ...
        try:
            ...
            result = self.db.query(MaintenanceModel).filter(
                and_(
                    MaintenanceModel.status.in_([
                        MaintenanceStatus.PENDING,
                        MaintenanceStatus.IN_PROGRESS
                    ]),
                    MaintenanceModel.updated_at < cutoff_time
                )
            ).update({
                'status': MaintenanceStatus.CANCELLED,
                'updated_at': datetime.now(timezone.utc),
                'notes': func.concat(
                    MaintenanceModel.notes,
    ...
    async def validate_maintenance_data(self, data: Dict[str, Any], is_update: bool = False) -> List[str]:
        """
        ...
        for field, max_length in max_lengths.items():
            if field in data and isinstance(data[field], str):
                if len(data[field]) > max_length:
                    errors.append(f"{field} 필드가 최대 길이({max_length})를 초과했습니다")
    ...
    async def check_maintenance_permission(
        ...
        try:
            ...
            # 차량 담당자 확인
            is_vehicle_manager = await self.db.query(VehicleManagerModel).filter(
                and_(
            ...
            # 관리자 권한 확인
            is_admin = await self.db.query(UserModel).filter(
                and_(
        ...
        try:
            ...
            if new_status == MaintenanceStatus.COMPLETED:
                ...
                if maintenance and maintenance.is_periodic:
                    next_date = maintenance.date + timedelta(days=maintenance.period_days)
    ...
    def trigger_maintenance_notification(
        ...
        try:
            ...
            # 관련 사용자 조회
            vehicle_managers = self.db.query(VehicleManagerModel).filter(
                VehicleManagerModel.vehicle_id == maintenance.vehicle_id
            ).all()
    ...
    def _should_create_index(self, index_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            return (
                cost_benefit['performance_improvement'] >= thresholds['min_query_improvement']
    ...
    def _get_optimal_cache_config(self, query_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """최적의 캐시 설정을 결정합니다."""
        try:
            # 쿼리 특성 분석
            characteristics = {
            ...
            # 캐시 전략 결정
            if characteristics['read_frequency'] > characteristics['write_frequency'] * 5:
                # 읽기가 많은 경우
                return {
                    'cache_type': 'full',
                    'ttl': 3600,
        ...
        try:
            ...
            self.dashboard_manager.configure_layout([
                ...
                },
                {
                    'name': '정비 현황',
                    'type': 'grid',
                    'panels': ['maintenance_status', 'completion_rate', 'cost_trend']
                },
                {
                    'name': '성능 모니터링',
                    'type': 'grid',
    ...
    async def get_dashboard_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 성능 메트릭 수집
            performance_metrics = await self.dashboard_manager.collect_performance_metrics()
    ...
    @track_db_query_time
    async def get_recent_maintenance_records(self, limit: int = 10) -> List[Dict[str, Any]]:
        """최근 정비 기록 조회"""
        try:
            records = (
                self.db.query(MaintenanceModel)
    ...
    async def get_pending_approval_maintenance(self) -> List[Dict[str, Any]]:
        ...
        try:
            records = (
                self.db.query(MaintenanceModel)
                .filter(MaintenanceModel.status == 'pending_approval')
                .order_by(MaintenanceModel.date.desc())
                .all()
            )
    ...
    async def count_maintenance_by_status(self, status: str) -> int:
        ...
        try:
            count = (
                self.db.query(func.count(MaintenanceModel.id))
                .filter(MaintenanceModel.status == status)
                .scalar() or 0
            )
