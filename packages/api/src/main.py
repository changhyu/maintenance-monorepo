...
# packagescore 유틸리티 모듈 임포트
from packagescore.utils import (
    CustomJSONEncoder, generate_etag, check_etag, 
    paginate_list, to_dict, to_json
)
...
app = FastAPI(
    ...
    # JSON 응답 처리를 위한 custom encoder 설정
...
async def etag_middleware(request: Request, call_next):
    ...
    if request.url.path.startswith("/api/") and request.method == "GET":
        # 응답 데이터가 있을 경우 ETag 생성
        if hasattr(response, "body"):
            try:
                # 응답 본문을 파싱하여 ETag 헤더 추가
                body = response.body.decode()
                response.headers["ETag"] = f'"{generate_etag(body)}"'
            except Exception as e:
                logger.error(f"ETag 생성 중 오류: {e}")
...
async def system_info():
    ...
    return JSONResponse(content=json.loads(json_info))
...
async def startup_event():
    ...
    logger.info("packagescore.utils 모듈이 성공적으로 통합되었습니다.")
