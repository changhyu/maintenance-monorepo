...
from ..models.schedule import ScheduleModel, ScheduleStatus
...
class ScheduleController:
    ...
    async def get_schedules(
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"일정을 조회할 수 없습니다: {str(e)}") from e
    ...
    async def create_schedule(self, schedule_data: ScheduleCreate) -> ScheduleResponse:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"일정을 생성할 수 없습니다: {str(e)}") from e
    
    async def update_schedule(self, schedule_id: str, schedule_data: ScheduleUpdate) -> ScheduleResponse:
        """
        일정 수정
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"일정을 수정할 수 없습니다: {str(e)}") from e
    ...
    async def delete_schedule(self, schedule_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"일정을 삭제할 수 없습니다: {str(e)}") from e
    
    async def add_schedule_note(self, schedule_id: str, note_data: ScheduleNoteCreate) -> ScheduleNoteResponse:
        """
        일정에 노트 추가
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"노트를 추가할 수 없습니다: {str(e)}") from e
    
    @ttl_cache(ttl=180)  # 3분 캐싱
    async def get_upcoming_schedules(self, days: int = Query(7, ge=1, le=30)) -> ScheduleListResponse:
        """
        다가오는 일정 조회
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"다가오는 일정을 조회할 수 없습니다: {str(e)}") from e
    ...
    async def get_schedules_by_vehicle(self, vehicle_id: str) -> ScheduleListResponse:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"차량별 일정을 조회할 수 없습니다: {str(e)}") from e
    
    async def change_schedule_status(self, schedule_id: str, status_data: ScheduleStatusUpdate) -> ScheduleResponse:
        """
        일정 상태 변경
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"일정 상태를 변경할 수 없습니다: {str(e)}") from e
    
    @ttl_cache(ttl=60)  # 1분 캐싱
    async def get_schedule_reminders(self, schedule_id: str) -> List[ScheduleReminderResponse]:
        """
        일정 알림 조회
        ...
        try:
            ...
            return [ScheduleReminderResponse.model_validate(reminder) for reminder in reminders]
        except NotFoundException as e:
            raise HTTPException(status_code=404, detail=str(e)) from e
        except Exception as e:
            logger.error(f"일정 알림 조회 중 오류 발생: {str(e)}")
            raise HTTPException(status_code=500, detail=f"일정 알림을 조회할 수 없습니다: {str(e)}") from e
    ...
    async def get_schedule_notes(self, schedule_id: str) -> List[ScheduleNoteResponse]:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"일정 노트를 조회할 수 없습니다: {str(e)}") from e
    ...
    async def get_overdue_schedules(self) -> ScheduleListResponse:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"기한 초과 일정을 조회할 수 없습니다: {str(e)}") from e
    
    @ttl_cache(ttl=300)  # 5분 캐싱
    async def get_maintenance_statistics(self, vehicle_id: Optional[str] = None) -> Dict[str, Any]:
        """
        정비 통계 정보 조회
    ...
    def _process_vehicle_forecast(self, vehicle: VehicleModel, today: datetime, db: Session) -> Dict[str, Any]:
        """단일 차량의 정비 예측 정보를 처리"""
        try:
            last_maintenance = db.query(ScheduleModel).filter(
                ScheduleModel.vehicle_id == vehicle.id,
                ScheduleModel.status == "completed"
            ).order_by(
                ScheduleModel.scheduled_date.desc()
            ).first()
    ...
    @ttl_cache(ttl=300)  # 5분 캐싱
    async def get_scheduled_maintenance_forecast(self, days: int = 90) -> List[Dict[str, Any]]:
        """
        향후 정비 예측 조회
        ...
        try:
            ...
            # 각 차량의 예측 정보 처리
            result = [self._process_vehicle_forecast(vehicle, today, db) for vehicle in vehicles]
            
            # 정렬 함수 정의
            def get_next_date(item: Dict[str, Any]) -> datetime:
                return item.get("next_maintenance_date") or (end_date + timedelta(days=100))
            ...
            raise HTTPException(status_code=500, detail=f"정비 예측을 조회할 수 없습니다: {str(e)}") from e
    ...
    def process_schedule_action(self, schedule_id: str, action: str) -> Optional[Dict[str, Any]]:
        """일정 액션 처리"""
