...
class TestMaintenanceService:
    ...
    @pytest.mark.asyncio
    async def test_create_maintenance_record(self, maintenance_service, sample_maintenance_record):
        """정비 기록 생성 테스트"""
        # Given
        mock_db = Mock()
        mock_db.maintenance_records.create.return_value = MaintenanceRecord(**sample_maintenance_record)
        maintenance_service.db = mock_db

        # When
        result = await maintenance_service.create_maintenance_record(sample_maintenance_record)
    ...
    @pytest.mark.asyncio
    async def test_get_maintenance_record(self, maintenance_service, sample_maintenance_record):
        """정비 기록 조회 테스트"""
        # Given
        mock_db = Mock()
        mock_db.maintenance_records.get.return_value = MaintenanceRecord(**sample_maintenance_record)
        maintenance_service.db = mock_db

        # When
        result = await maintenance_service.get_maintenance_record(sample_maintenance_record['id'])

        # Then
        assert result.id == sample_maintenance_record['id']
        mock_db.maintenance_records.get.assert_called_once_with(sample_maintenance_record['id'])
    ...
    @pytest.mark.asyncio
    async def test_update_maintenance_record(self, maintenance_service, sample_maintenance_record):
        """정비 기록 업데이트 테스트"""
        # Given
        mock_db = Mock()
        updated_record = sample_maintenance_record.copy()
        updated_record['status'] = MaintenanceStatus.COMPLETED
        mock_db.maintenance_records.update.return_value = MaintenanceRecord(**updated_record)
        maintenance_service.db = mock_db
    ...
    @pytest.mark.asyncio
    async def test_delete_maintenance_record(self, maintenance_service, sample_maintenance_record):
        """정비 기록 삭제 테스트"""
        # Given
        mock_db = Mock()
        ...
        # When
        result = await maintenance_service.delete_maintenance_record(sample_maintenance_record['id'])

        # Then
        assert result is True
        mock_db.maintenance_records.delete.assert_called_once_with(sample_maintenance_record['id'])

    @pytest.mark.asyncio
    async def test_get_vehicle_maintenance_history(self, maintenance_service, sample_maintenance_record):
        """차량별 정비 이력 조회 테스트"""
        # Given
        mock_db = Mock()
    ...
    @pytest.mark.asyncio
    async def test_schedule_maintenance(self, maintenance_service, sample_maintenance_record):
        """정비 일정 예약 테스트"""
        # Given
        mock_db = Mock()
        scheduled_record = sample_maintenance_record.copy()
        scheduled_record['date'] = datetime.now() + timedelta(days=7)
        mock_db.maintenance_records.create.return_value = MaintenanceRecord(**scheduled_record)
        maintenance_service.db = mock_db
    ...
    @pytest.mark.asyncio
    async def test_invalid_maintenance_date(self, maintenance_service, sample_maintenance_record):
        """과거 날짜로 정비 예약 시도 테스트"""
        # Given
