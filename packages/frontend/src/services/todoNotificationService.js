import { TodoPriority } from './todoService';
...
(function (NotificationType) {
    ...
    NotificationType["PRIORITY_HIGH"] = "priority_high";
...
class TodoNotificationService {
    ...
    cleanOldNotifications() {
        ...
        // 만료된 알림 제거
        let unexpiredNotifications = this.notifications.filter(notification => !notification.expiry || notification.expiry > now);
     ...
     * Todo 상태 변경 알림
     */
    notifyStatusChange(todo, previousCompleted) {
     ...
     * 마감일 임박 알림 확인
     */
    checkAndNotifyUpcomingDue(todos) {
        try {
            ...
            upcoming.forEach(todo => {
                ...
                if (!notificationSent) {
                    ...
                    // 데스크톱 알림 표시
                    this.showDesktopNotification('마감일 임박', `작업 "${todo.title}"의 마감일이 ${formatDate(todo.dueDate ?? '')}로 임박했습니다.`);
                }
            });
        }
        catch (error) {
        ...
        try {
            ...
            overdue.forEach(todo => {
                ...
                if (!notificationSent) {
                    ...
                    this.sentNotifications.add(notificationKey);
                    // 데스크톱 알림 표시
                    this.showDesktopNotification('마감일 초과', `작업 "${todo.title}"의 마감일(${formatDate(todo.dueDate ?? '')})이 지났습니다.`);
                }
            });
        }
        catch (error) {
     ...
     */
    mapNotificationsArray(notifications) {
