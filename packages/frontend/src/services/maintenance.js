...
export const maintenanceService = {
     ...
     * @returns 정비 이력 목록
     */
    async getVehicleMaintenanceHistory(vehicleId, filter) {
        try {
            const params = filter ? { ...filter } : {};
     ...
     * @returns 첨부된 문서 정보
     */
    async attachDocument(recordId, file) {
        try {
     ...
     * @returns 성공 여부
     */
    async removeDocument(recordId, documentId) {
        try {
     ...
     * @returns 권장 정비 항목 목록
     */
    async getRecommendedMaintenance(vehicleId) {
        try {
