...
type ConnectionStats = {
  ...
  uptime: number; // 밀리초 단위
  isStable: boolean; // 잦은 재연결 없이 안정적인지 여부
};
  ...
  const handleStatusChange = useCallback((newStatus: SocketStatus) => {
    ...
    setStats(prev => {
      ...
      if (newStatus === SocketStatus.CONNECTED) {
        ...
        (newStatus === SocketStatus.DISCONNECTED || newStatus === SocketStatus.ERROR) && 
      ...
      const isFrequentReconnect = 
        newStats.reconnectCount >= 3 && 
