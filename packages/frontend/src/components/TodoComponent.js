...
const adaptFilterToContextFilter = (filter) => {
    let completed = 'all';
    if (filter.completed) {
...
const adaptContextFilterToFilter = (contextFilter) => {
    return {
 ...
 */
const TodoComponent = memo(({ vehicleId, className = '', showCompleted = true, todos: optimisticTodos, isPending = false, onTodoClick, onCreateTodo, onUpdateTodo, onDeleteTodo, onToggleComplete, compact = false, pendingChanges = [], isOffline = false, maxHeight = '100%', emptyMessage = '할 일이 없습니다' }) => {
    ...
    const pendingItemIds = useMemo(() => pendingChanges.map(change => change.id), [pendingChanges]);
    ...
    // 투두 생성 핸들러
    const handleCreate = useCallback(async (todo) => {
        try {
            if (onCreateTodo) {
    ...
    // 투두 업데이트 핸들러
    const handleUpdate = useCallback(async (todoId, updates) => {
        try {
            if (onUpdateTodo) {
    ...
    return (_jsxs("div", { className: `todo-component ${className}`, children: [!compact && (_jsx(TodoFilter, { onFilterChange: handleFilterChange, initialFilter: filterState.currentFilter, className: "" })), _jsx(AddTodoForm, { onSubmit: handleCreate, vehicleId: vehicleId, disabled: isPending }), _jsx(Divider, { style: { margin: '16px 0' } }), _jsx(TodoList, { todos: filteredTodos, loading: isLoading, filterState: filterState, onTodoClick: onTodoClick, onToggleComplete: handleToggleComplete, onUpdateTodo: handleUpdate, onDeleteTodo: handleDelete })] }));
});
// 디스플레이 이름 설정
