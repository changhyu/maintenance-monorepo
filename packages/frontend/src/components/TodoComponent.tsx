...
const TodoComponent: React.FC<TodoComponentProps> = ({
  ...
  const { todos, loading, error, fetchTodos, createTodo, updateTodo, deleteTodo, toggleComplete } =
    useTodoService();

  const [filterState, filterDispatch] = useFilterState();
  const [templateState, templateDispatch] = useTemplateState();

  // TodoFilter를 TodoFilterType으로 변환하는 함수
  const adaptFilterToContextFilter = (filter: FilterType) => {
    return {
      completed: filter.completed ? 'completed' as const : 'all' as const,
      priority: Array.isArray(filter.priority) 
        ? filter.priority[0] || ('all' as const) 
        : filter.priority || ('all' as const),
  ...
  // TodoFilterType을 TodoFilter로 변환하는 함수
  const adaptContextFilterToServiceFilter = (contextFilter: any) => {
    return {
      completed: contextFilter.completed === 'completed',
      priority: contextFilter.priority !== 'all' ? contextFilter.priority : undefined,
      searchText: contextFilter.search || undefined,
  ...
  const handleCreateTodo = useCallback(
    async (todoData: TodoCreateRequest) => {
      try {
        // 외부에서 제공된 핸들러가 있다면 해당 핸들러 사용
        const newTodo = onCreateTodo ? await onCreateTodo(todoData) : await createTodo(todoData);
    ...
    async (id: string, updates: TodoUpdateRequest) => {
      try {
        ...
        if (updatedTodo) {
          ...
          if (
            prevTodo.priority !== TodoPriority.HIGH &&
            updatedTodo.priority === TodoPriority.HIGH
          ) {
  ...
  const handleToggleComplete = useCallback(
    async (id: string) => {
      try {
        ...
        // 외부에서 제공된 핸들러가 있다면 해당 핸들러 사용
        const result = onToggleComplete ? await onToggleComplete(id) : await toggleComplete(id, !prevTodo.completed);
