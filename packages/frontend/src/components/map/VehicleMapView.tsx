...
const VehicleMapView: React.FC<VehicleMapViewProps> = ({
  ...
  // 수리점 선택 시 호출되는 함수
  const handleShopSelect = useCallback((shop: RepairShop) => {
  ...
  // 테스트용 더미 데이터 생성
  const generateDummyVehicles = (): Vehicle[] => {
    return [
  ...
  const generateDummyLocations = (vehicles: Vehicle[]): VehicleLocation[] => {
  ...
  const generateDummyRepairShops = (): RepairShop[] => {
    return [
      { id: '1', name: '서울 중앙 정비소', address: '서울시 중구 123', latitude: initialCenter.lat + 0.02, longitude: initialCenter.lng + 0.02, rating: 4.5, specialties: ['트럭', '버스'] },
      { id: '2', name: '강남 모터스', address: '서울시 강남구 456', latitude: initialCenter.lat - 0.03, longitude: initialCenter.lng + 0.01, rating: 4.2, specialties: ['승용차', '밴'] },
      { id: '3', name: '종로 자동차 센터', address: '서울시 종로구 789', latitude: initialCenter.lat + 0.01, longitude: initialCenter.lng - 0.03, rating: 4.7, specialties: ['트럭', '버스', '밴'] },
      { id: '4', name: '마포 정비 공장', address: '서울시 마포구 101', latitude: initialCenter.lat - 0.02, longitude: initialCenter.lng - 0.02, rating: 3.9, specialties: ['트럭', '특수차량'] },
    ];
  };
    ...
    >
      <div style={{ position: 'relative', width: '100%', height: '600px' }}>
        ...
        >
                  ...
                  >
                    <div style={{ padding: '5px' }}>
                      ...
                      <Space direction="vertical" size="small">
                        <Text><CarOutlined /> 종류: {vehicle.type}</Text>
                        <Text>
                          상태: {' '}
                          <Badge 
                            status={
                            ...
                            text={
                              vehicle.status === 'active' ? '운행 중' : 
                              vehicle.status === 'maintenance' ? '정비 중' : '비활성'
                            } 
                          />
                        </Text>
                        <Text>상태 점수: {vehicle.healthScore}%</Text>
                        <Text>최근 업데이트: {new Date(location.lastUpdated).toLocaleString()}</Text>
                        {showRepairShops && (
                          <Button 
                            type="primary" 
                            size="small"
          ...
          {/* 수리점 마커 */}
          {showRepairShops && repairShops.map(shop => (
            <Marker
              key={`shop-${shop.id}`}
