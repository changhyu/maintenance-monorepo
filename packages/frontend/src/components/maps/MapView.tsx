...
// 주변 검색 옵션 인터페이스
interface NearbySearchOptions extends SearchRadius {
  center: Coordinates;
  radius: number;
  unit: DistanceUnit;
}
...
const MapView = ({
  apiClient,
  googleMapsApiKey,
  initialCenter = { latitude: 37.5665, longitude: 126.978 }, // 서울 중심
  initialZoom = 13,
  vehicleId,
  userId,
  showShops = true,
  ...
  // 경계 내 정비소 로드
  const loadShopsInBounds = async (bounds: MapBounds) => {
    try {
      setLoading(true);
  ...
  const findNearbyShops = useCallback(
    async (location: Coordinates) => {
      ...
      try {
        setLoading(true);
        const searchOptions: SearchRadius = {
          center: location,
          radius: searchRadius,
  ...
  if (loadError) {
    return (
      <Card>
        <Text type="danger">지도를 로드하는 데 실패했습니다: {loadError.message}</Text>
      </Card>
    );
  }
            ...
            {selectedVehicle && (
              ...
              >
                <div>
                  ...
                  <p>
                    <strong>주소:</strong> {selectedVehicle.address}
                  </p>
                  <p>
                  ...
                  {selectedVehicle.speed !== undefined && (
                    <p>
                      <strong>속도:</strong> {selectedVehicle.speed} km/h
                    </p>
                  )}
            ...
            {selectedShop && (
              ...
              >
                <div>
                  ...
                  <p>
                    <strong>주소:</strong> {selectedShop.address}
                  </p>
                  <p>
                    <strong>평점:</strong> <Rate disabled defaultValue={selectedShop.rating} /> (
                    {selectedShop.rating})
                  </p>
                  <p>
                    <strong>영업시간:</strong> {selectedShop.openHours}
                  </p>
                  <p>
                    <strong>연락처:</strong> {selectedShop.contactNumber}
                  </p>
                  <p>
                    <strong>서비스:</strong> {selectedShop.services.join(', ')}
                  </p>
        ...
        {route && (
          <Card title="경로 정보" className="mt-4">
            <Space direction="vertical">
              <Text>
                <strong>출발지:</strong> {route.origin.address}
              </Text>
              <Text>
                <strong>목적지:</strong> {route.destination.address}
              </Text>
              <Text>
                <strong>총 거리:</strong> {route.distance} km
              </Text>
              <Text>
                <strong>예상 소요 시간:</strong> {Math.floor(route.duration / 60)}분
              </Text>
