...
export const useTodoService = () => {
  ...
  const fetchTodos = useCallback(
    async (filter?: TodoFilterType): Promise<Todo[]> => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const error =
          err instanceof Error ? err : new Error('할 일 목록을 불러오는 데 실패했습니다.');
        setError(error);
        throw error;
      } finally {
        setLoading(false);
  ...
  const createTodo = useCallback(
    async (todoData: TodoCreateRequest): Promise<Todo> => {
      setLoading(true);
      setError(null);
  ...
  const updateTodo = useCallback(
    async (id: string, updates: TodoUpdateRequest): Promise<Todo> => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const error =
          err instanceof Error ? err : new Error('할 일을 업데이트하는 데 실패했습니다.');
        setError(error);
        throw error;
      } finally {
        setLoading(false);
