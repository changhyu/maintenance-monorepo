...
export const useIndexedDB = <T>(storeName: string) => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    setError(null);
    try {
      const items = await indexedDBUtils.getAllData<T>(storeName);
      setData(items);
      return items;
    } catch (err) {
  ...
  const getItem = useCallback(async (id: string): Promise<T | null> => {
    ...
    setError(null);
    try {
      return await indexedDBUtils.getData<T>(storeName, id);
    } catch (err) {
      const error = err instanceof Error ? err : new Error(String(err));
      setError(error);
  ...
  // 인덱스를 통한 데이터 불러오기
  const getByIndex = useCallback(async (indexName: string, value: any): Promise<T[]> => {
    setIsLoading(true);
    setError(null);
    try {
      const items = await indexedDBUtils.getDataByIndex<T>(storeName, indexName, value);
      return items;
    } catch (err) {
      const error = err instanceof Error ? err : new Error(String(err));
      setError(error);
  ...
  // 항목 저장 (추가 또는 업데이트)
  const saveItem = useCallback(async (item: any): Promise<string> => {
    setIsLoading(true);
    setError(null);
    try {
...
export const useOfflineMode = () => {
  ...
  // 오프라인 모드 설정
  const setOfflineMode = useCallback(async (newOfflineStatus: boolean) => {
    try {
...
export const usePendingOperations = () => {
  ...
  // 작업 큐에 추가
  const queueOperation = useCallback(async (method: string, url: string, data?: any) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  // 작업 큐에서 제거
  const removeOperations = useCallback(async (operationIds: string[]) => {
    setIsLoading(true);
    setError(null);
    try {
...
export const useTodoCache = () => {
  const { 
    ...
    deleteItem, 
    clearAll 
  } = useIndexedDB(indexedDBUtils.STORES.TODOS);
  ...
  // Todo 항목 캐싱
  const cacheTodo = useCallback(async (todo: any) => {
    return await saveItem(todo);
  }, [saveItem]);

  // Todo 항목 조회
  const getCachedTodo = useCallback(async (todoId: string) => {
    return await getItem(todoId);
  }, [getItem]);

  // 특정 차량의 Todo 항목 조회
  const getTodosByVehicleId = useCallback(async (vehicleId: string) => {
    return await getByIndex('vehicleId', vehicleId);
  }, [getByIndex]);

  // 완료/미완료 Todo 항목 조회
  const getTodosByCompletion = useCallback(async (completed: boolean) => {
    return await getByIndex('completed', completed);
  }, [getByIndex]);

  // 특정 기한의 Todo 항목 조회
  const getTodosByDueDate = useCallback(async (dueDate: string | Date) => {
...
export const useVehicleCache = () => {
  const { 
    ...
    deleteItem, 
    clearAll 
  } = useIndexedDB(indexedDBUtils.STORES.VEHICLES);
  ...
  // 차량 정보 캐싱
  const cacheVehicle = useCallback(async (vehicle: any) => {
    return await saveItem(vehicle);
  }, [saveItem]);

  // 차량 정보 조회
  const getCachedVehicle = useCallback(async (vehicleId: string) => {
    return await getItem(vehicleId);
  }, [getItem]);

  // 사용자의 차량 목록 조회
  const getVehiclesByUserId = useCallback(async (userId: string) => {
    return await getByIndex('userId', userId);
  }, [getByIndex]);

  // 제조사별 차량 목록 조회
  const getVehiclesByMake = useCallback(async (make: string) => {
    return await getByIndex('make', make);
  }, [getByIndex]);

  // 연식별 차량 목록 조회
  const getVehiclesByYear = useCallback(async (year: number) => {
...
export const useUserSettings = () => {
  const { 
    ...
    saveItem, 
    clearAll 
  } = useIndexedDB(indexedDBUtils.STORES.USER_SETTINGS);
