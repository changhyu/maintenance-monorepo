...
export const useBookingService = () => {
    ...
    // 고객 ID로 예약 조회
    const getCustomerBookings = async (customerId, filter) => {
        if (!customerId) {
    ...
    const cancelBooking = async (bookingId, reason) => {
        ...
        try {
            ...
            if (cancelledBooking) {
                setBookings(prev => prev.map(booking => (booking.id === cancelledBooking.id ? cancelledBooking : booking)));
    ...
    // 가용 시간대 조회
    const getAvailableTimeSlots = async (shopId, date, serviceType) => {
        if (!shopId) {
