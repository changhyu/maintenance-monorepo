...
const mockSystemInfo: SystemInfo = {
  ...
  current_directory: "/Users/user/Desktop/project",
...
const mockCpuInfo: CpuInfo = {
  physical_cores: 8,
  total_cores: 8,
  max_frequency: "3.2 GHz",
  current_frequency: "2.4 GHz",
  cpu_usage_per_core: ["12%", "15%", "8%", "10%", "5%", "7%", "9%", "11%"],
  total_cpu_usage: "9.6%"
};

const mockProcesses: ProcessInfo[] = [
  { pid: 1234, name: "node", username: "user", memory_percent: "2.5%", cpu_percent: "1.2%", status: "running" },
  { pid: 2345, name: "python", username: "user", memory_percent: "1.8%", cpu_percent: "0.9%", status: "running" },
  { pid: 3456, name: "chrome", username: "user", memory_percent: "15.2%", cpu_percent: "8.7%", status: "running" },
  { pid: 4567, name: "mysqld", username: "mysql", memory_percent: "3.1%", cpu_percent: "1.5%", status: "running" },
  { pid: 5678, name: "nginx", username: "www-data", memory_percent: "0.8%", cpu_percent: "0.3%", status: "sleeping" }
];
...
const mockLogFiles: LogFile[] = [
  { filename: "app.log", size: 1024 * 1024, modified: new Date().toISOString(), created: new Date(Date.now() - 86400000).toISOString() },
  { filename: "error.log", size: 512 * 1024, modified: new Date().toISOString(), created: new Date(Date.now() - 86400000).toISOString() },
  { filename: "access.log", size: 2048 * 1024, modified: new Date().toISOString(), created: new Date(Date.now() - 86400000).toISOString() }
];
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setCpuInfo(mockCpuInfo);
        setDiskInfo(mockDiskInfo);
      ...
      */
      ...
      // setLoading(false); // setTimeout에서 처리
    }
  };
  ...
  const logFileColumns = [
    {
      ...
      render: (text: string) => (
        <Button 
          type="link" 
          onClick={() => alert(`로그 파일 '${text}' 내용을 표시합니다.`)}
        >
          {text}
        </Button>
      ),
