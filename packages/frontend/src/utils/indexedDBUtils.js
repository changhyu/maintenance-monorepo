...
export const saveData = async (storeName, data) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onerror = (event) => {
            console.error(`데이터 저장 실패 (${storeName}):`, event.target.error);
            reject(event.target.error);
        };
...
export const getData = async (storeName, key) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onerror = (event) => {
            console.error(`데이터 조회 실패 (${storeName}):`, event.target.error);
            reject(event.target.error);
        };
        transaction.oncomplete = () => db.close();
    });
...
export const getDataByIndex = async (storeName, indexName, value) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onerror = (event) => {
            console.error(`인덱스 조회 실패 (${storeName}.${indexName}):`, event.target.error);
            reject(event.target.error);
        };
        transaction.oncomplete = () => db.close();
    });
...
export const deleteData = async (storeName, key) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onerror = (event) => {
            console.error(`데이터 삭제 실패 (${storeName}):`, event.target.error);
            reject(event.target.error);
        };
        transaction.oncomplete = () => db.close();
    });
...
export const clearStore = async (storeName) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onerror = (event) => {
            console.error(`저장소 초기화 실패 (${storeName}):`, event.target.error);
            reject(event.target.error);
        };
        transaction.oncomplete = () => db.close();
    });
 ...
 * @returns Promise<void>
 */
export const removeOperationsFromQueue = async (operationIds) => {
    try {
        const db = await initDatabase();
        ...
        const promises = operationIds.map(id => new Promise((resolve, reject) => {
 ...
 */
export const cacheTodo = async (todo) => {
 ...
 */
export const getCachedTodo = async (id) => {
 ...
 */
export const getCachedTodosByVehicle = async (vehicleId) => {
    ...
    try {
        ...
        for (const store of Object.values(STORES)) {
            try {
                ...
                result.stores[store] = { error: error.message };
            }
        }
        db.close();
        return result;
 ...
 * @returns 병합된 항목 수
 */
export const mergeStores = async (sourceStore, targetStore) => {
    try {
        // 유효한 스토어인지 확인
        const storeNames = Object.values(STORES);
        if (!storeNames.includes(sourceStore) || !storeNames.includes(targetStore)) {
        ...
        await initDatabase();
        // 원본 스토어의 모든 데이터 가져오기
        const sourceData = await getAllData(sourceStore);
        const targetData = await getAllData(targetStore);
        ...
        let mergedCount = 0;
        // 타입 안전성을 위한 타입 가드 함수
        function isValidStoreItem(item) {
 ...
 */
export const saveReportSchedule = async (schedule) => {
    try {
        // ID가 없는 경우 자동 생성
        if (!schedule.id) {
 ...
 */
export const calculateNextRunDate = (schedule) => {
    const now = new Date();
