 ...
 */
export const formatRelativeTime = (date, baseDate) => {
    try {
 ...
 */
export const isPastDate = (date, baseDate = new Date()) => {
    try {
        // 시간 정보 제거하고 날짜만 비교
 ...
 * @returns 더 최근 날짜 문자열
 */
export const getMoreRecentDate = (date1, date2) => {
    try {
        if (!date1)
 ...
 */
export const isValidDate = (dateString, minDate, maxDate) => {
    try {
        // 기본 날짜 형식 검증
 ...
 * @returns 유효하면 true, 아니면 false
 */
export const isFutureDateTime = (dateString, timeString, minimumMinutesInFuture = 0) => {
    try {
