 ...
 */
export function isValidYear(year: number | undefined | null): boolean {
 ...
 * 차량 번호판 유효성 검사 (한국 기준)
 */
export function isValidLicensePlate(plate: string | undefined | null): boolean {
  ...
  // 구형 번호판 (12가3456)
  if (/^\d{2}[가-힣]\d{4}$/.test(cleaned)) return true;
  
  // 신형 번호판 (123가4567)
  if (/^\d{3}[가-힣]\d{4}$/.test(cleaned)) return true;
