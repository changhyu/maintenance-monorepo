...
# 모든 문제 해결 최종 스크립트 (모든 버전 충돌 포함)

echo "========== 프로젝트 모든 문제 최종 해결 ==========\n"

# 스크립트에 실행 권한 부여
echo "1. 스크립트에 실행 권한 부여..."
chmod +x fix_database.py fix_all_issues.py fix_config.py fix_npm_deps.sh fix_port.py restart_project.sh fix_concurrently.sh fix_npm_deps_force.sh fix_prisma_version.sh fix_file_saver_types.sh
