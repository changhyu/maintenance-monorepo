...
@router.post(
    f"{settings.API_V1_STR}/token", response_model=Token,
    summary="액세스 토큰 발급"
)
...
@router.post(
    "/api/admin/login", response_model=Dict[str, Any],
    summary="관리자 로그인"
)
async def admin_login(
    ...
    # 관리자 역할 확인 (role이 ADMIN인 경우만 관리자로 허용)
...
@router.get(
    "/api/admin/me", response_model=Dict[str, Any],
...
async def get_admin_me(
    ...
    return {
        "user": {
            ...
            "role": user.role.value,
            "is_active": user.is_active,
            # 추가 필요한 관리자 정보가 있다면 여기에 포함
            "permissions": ["admin:read", "admin:write", "user:read", "user:write"] 
            # 실제로는 토큰에서 권한 정보를 가져오거나 역할에 따라 권한을 조회해야 함
        }
    }